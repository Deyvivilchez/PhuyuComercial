<div id="phuyu_buscar">
	<div style="padding:10px 0px; height:53px; border-bottom: 2px solid #f3f3f3;">
		<div class="col-md-12 col-xs-10">
			<input type="text" class="form-control" v-model="buscar" v-on:keyup="phuyu_buscar()" placeholder="BUSCAR PROFORMA . . ." v-bind:autofocus="true">
		</div>
	</div>
	...
	<div class="col-xs-12">
		...
		<div class="row" v-if="!cargando">
			<table class="table table-striped projects">
				<tbody>
					<tr v-for="dato in proformas">
						<td style="width:60%;cursor:pointer;" v-on:click="phuyu_seleccionado(dato)">
							<a><strong>{{dato.documento}}</strong>  {{dato.razonsocial}}</a> <br> 
	...
	var phuyu_buscar = new Vue({
		...
		methods: {
			phuyu_pedidos: function(){
				...
				this.cargando = true;
				this.$http.post(url+"compras/proformas/buscar_lista",{"buscar":this.buscar,"pagina":this.paginacion.actual}).then(function(data){
					this.proformas = data.body.lista; this.paginacion = data.body.paginacion; this.cargando = false;
				},function(){
			...
			phuyu_seleccionado: function(proforma){
				phuyu_operacion.phuyu_addproforma(proforma);
			},
