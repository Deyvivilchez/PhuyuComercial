...
<div id="phuyu_datos" class="phuyu-velzon-list">
	<div class="phuyu-page-title">
		<div class="phuyu-page-icon"><i class="bi bi-box-seam"></i></div>
		<div>
			<div class="text-muted small text-uppercase fw-semibold">Administracion</div>
			<h4 class="mb-0 fw-bold">Almacenes</h4>
			...
			<div class="card-body">
				<div class="phuyu-toolbar">
					<div class="phuyu-search">
						...
						<input class="form-control datatable-search" v-model="buscar" v-on:keyup="phuyu_buscar()" placeholder="Buscar almacen">
					</div>
					...
					<table class="table table-hover table-striped align-middle">
						...
						<tbody>
							<tr v-for="dato in datos">
								<td class="fw-semibold">{{dato.codalmacen}}</td>
								<td>{{dato.descripcion}}</td>
								<td>{{dato.sucursal}}</td>
								<td>{{dato.telefonos}}</td>
								<td class="text-center">
									<input type="radio" class="form-check-input" name="phuyu_seleccionar" v-on:click="phuyu_seleccionar(dato.codalmacen)">
								</td>
							</tr>
							<tr v-if="datos.length==0 && !cargando">
								<td colspan="5" class="text-center text-muted py-4">
									<i class="bi bi-inbox me-1"></i> Sin almacenes registrados
								</td>
							</tr>
						</tbody>
					</table>
