...
<div id="phuyu_datos" class="phuyu-velzon-list">
	<div class="phuyu-page-title">
		<div class="phuyu-page-icon"><i class="bi bi-people"></i></div>
		<div>
			...
			<div class="card-body">
				<div class="phuyu-toolbar">
					<div class="phuyu-search">
						...
						<input class="form-control datatable-search" v-model="buscar" v-on:keyup="phuyu_buscar()" placeholder="Buscar usuario">
					</div>
					...
					<div class="phuyu-actions">
						...
						<button type="button" class="btn btn-info" v-on:click="phuyu_asignarzona()">
							<i class="bi bi-geo-alt me-1"></i> Asignar zonas
						</button>
					</div>
				</div>
					...
					<table class="table table-hover table-striped align-middle">
						...
						<tbody>
							<tr v-for="dato in datos">
								<td class="fw-semibold">{{dato.codusuario}}</td>
								<td>{{dato.razonsocial}}</td>
								<td>{{dato.usuario}}</td>
								<td>
									<template v-for="s in dato.sucursales">
										<span class="badge bg-danger-subtle text-danger">{{s.sucursal}}</span>&nbsp;
									</template>
								</td>
								<td>{{dato.perfil}}</td>
								<td class="text-center">
									<input type="radio" class="form-check-input" name="phuyu_seleccionar" v-on:click="phuyu_seleccionar(dato.codusuario)">
								</td>
							</tr>
