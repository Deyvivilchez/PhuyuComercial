				...
				<tbody>
					<tr v-for="(dato,index) in pedidos">
						...
						<td style="width:40%;" align="center">
							<b style="font-size:16px;">S/. {{dato.importe}}</b><br>
							<button type="button" v-if="verprecios==1" v-on:click="phuyu_detallepedido(dato)" class="btn btn-success btn-xs"> <b>VER DETALLE</b> </button>
						</td>
					</tr>
				</tbody>
			</table>
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<div class="modal-header modal-phuyu-titulo">
					<h5 class="modal-title"><b style="font-size:20px;">DETALLE DEL PEDIDO</b></h5>
					<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
				</div>
				<div class="modal-body" id="cuerpopedido">
...
<script>
	var phuyu_buscar = new Vue({
		el: "#phuyu_buscar",
		data: {
			cargando: true, buscar: "", verprecios:1, detallepedido:[],totales:[],
			pedidos:[],
		...
		methods: {
			...
			phuyu_detallepedido: function(dato){
				$("#modal_stock").modal("show");
				this.$http.get(url+"ventas/pedidos/ver/"+dato.codpedido).then(function(data){
					$("#cuerpopedido").empty().html(data.body);
				});
			},
