		...
		if ($this->input->is_ajax_request()) {
			...
			foreach ($info["habitaciones"] as $habitacion) {
				$detalle[] = $this->Hotel_model->armar_item_kardex(
					...
					1, (double)$habitacion["subtotal"], "ALOJAMIENTO HAB. ".$habitacion["numero"]." - ".$habitacion["noches"]." NOCHE(S)",
			...
			$consumosPendientes = (double)$totales->importe - (double)$info["estadia"]["alojamiento"];
