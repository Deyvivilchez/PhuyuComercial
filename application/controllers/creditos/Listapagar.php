...
class Listapagar extends CI_Controller {
	...
	public function lista(){
		if ($this->input->is_ajax_request()) {
			...
			if($this->request->estado!=""){
			 ...
			 $programado = '';
			if($this->request->campos->creditoprogramado != ""){
				$programado = ' AND creditos.creditoprogramado='.$this->request->campos->creditoprogramado;
			}
			$lista = $this->db->query("select personas.razonsocial,personas.documento,creditos.codcredito,creditos.codkardex, creditos.fechacredito, creditos.fechavencimiento, round(creditos.importe,2) as importe, round(creditos.interes,2) as interes, round(creditos.saldo,2) as saldo, creditos.estado,round(creditos.total,2) as total from kardex.creditos as creditos inner join public.personas as personas on (creditos.codpersona=personas.codpersona) where (UPPER(personas.documento) like UPPER('%".$this->request->buscar."%') or UPPER(personas.razonsocial) like UPPER('%".$this->request->buscar."%') or UPPER(personas.nombrecomercial) like UPPER('%".$this->request->buscar."%') ) and creditos.codsucursal=".$_SESSION["phuyu_codsucursal"]." ".$estado." ".$programado." and ".$filtro." creditos.tipo=2 order by creditos.codcredito desc ")->result_array();
	...
	function phuyu_imprimirlistapdf(){
		if (isset($_SESSION["phuyu_usuario"])) {
			...
			$programado = '';
			if($this->request->creditoprogramado != ""){
				$programado = ' AND creditos.creditoprogramado='.$this->request->creditoprogramado;
			}
			
			$info = $this->db->query("select personas.razonsocial, personas.documento, creditos.codcredito,creditos.codkardex, creditos.fechacredito, creditos.fechavencimiento, round(creditos.importe,2) as importe, round(creditos.interes,2) as interes, round(creditos.saldo,2) as saldo,round(creditos.total,2) as total, creditos.estado from kardex.creditos as creditos inner join public.personas as personas on (creditos.codpersona=personas.codpersona) where creditos.codsucursal=".$_SESSION["phuyu_codsucursal"]." ".$estado." ".$programado." and ".$filtro."creditos.tipo=2 order by creditos.codcredito desc ")->result_array();
	        ...
	        $pdf->SetFont('Arial','B',10); 
	        $pdf->setFillColor(245,245,245); 
	        $pdf->Cell(0,7,"CREDITOS POR PAGAR - ".$tipo,1,1,'L',1);
	...
	function phuyu_imprimirlistaexcel(){
		if (isset($_SESSION["phuyu_usuario"])) {
			...
			$this->load->view("creditos/reportes/listageneralpagosxls",compact("info","total","tipo"));
		}
	}
	...
	function pdf_creditos(){
		if (isset($_SESSION["phuyu_usuario"])) {
		    ...
		    $lista = $this->db->query("select personas.razonsocial,personas.documento,creditos.* from kardex.creditos AS creditos inner join public.personas as personas on (creditos.codpersona=personas.codpersona) where creditos.codsucursal=".$_SESSION["phuyu_codsucursal"]." and creditos.estado=1 and creditos.tipo=2 order by creditos.codcredito desc")->result_array();
			...
			$pdf->SetTitle(utf8_decode("phuyu Perú - Creditos Pagar")); $pdf->Output();
		}else{
			$this->load->view("phuyu/505");
		}
	}
