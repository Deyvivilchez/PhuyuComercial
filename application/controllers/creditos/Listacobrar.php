...
class Listacobrar extends CI_Controller
{
	...
	function editar_guardar()
	{
		if ($this->input->is_ajax_request()) {
			...
			if (count($clave) > 0) {
				...
				$campos = ["codpersona"];
				$valores = [$this->request->campos->codpersona];
	...
	function phuyu_imprimir($codcredito)
	{
		if (isset($_SESSION["phuyu_usuario"])) {
			...
			$hora_s_2 = new DateTime($info[0]["fechavencimiento"]);
			...
			$pdf->SetWidths(array(10, 20, 20, 20, 20, 100));
			$pdf->SetLineHeight(5);
			...
			foreach ($cuotas as $key => $value) {
				...
				array_push($datos, utf8_decode($value["fechamovimiento"]));
				array_push($datos, "");
			...
			$pdf->SetTitle(utf8_decode("phuyu Perú - Credito"));
			$pdf->Output();
		}
	}
	...
	function phuyu_imprimirlistapdf()
	{
		if (isset($_SESSION["phuyu_usuario"])) {
			...
			$pdf->SetWidths(array(10, 18, 45, 16, 15, 21, 17, 15, 17, 16));
			$pdf->SetLineHeight(5);
			$pdf->SetFont('Arial', '', 7);
			$totalimporte = 0;
			$totalinteres = 0;
	...
	function pdf_creditos()
	{
		if (isset($_SESSION["phuyu_usuario"])) {
			...
			$pdf->SetWidths(array(10, 24, 75, 18, 17, 17, 16, 16));
			$pdf->SetLineHeight(5);
			$pdf->SetFont('Arial', '', 8);
			$item = 0;
			$importe = 0;
			...
			$pdf->Cell(16, 5, number_format($saldo, 2), 1, "R");
			$pdf->Ln();
			$pdf->Ln();

			$pdf->SetTitle(utf8_decode("phuyu Perú - Creditos Cobrar"));
			$pdf->Output();
		} else {
			$this->load->view("phuyu/505");
		}
	}
public function phuyu_imprimircronograma($codcredito){
	if (!isset($_SESSION["phuyu_usuario"])) {
		$this->load->view("phuyu/505");
		return;
	}
	...
	if ($codcredito <= 0) {
		show_error("Código de crédito inválido.", 400);
		return;
	}
	...
	$info = $this->db->query("
		SELECT 
			...
			p.direccion,
			p.telefono
		FROM kardex.creditos AS c
		INNER JOIN public.personas AS p ON (c.codpersona = p.codpersona)
		WHERE c.codcredito = ?
	", array($codcredito))->row_array();

	if (empty($info)) {
		show_error("No se encontró el crédito.", 404);
		return;
	}
	...
	foreach ($cuotas as $c) {
		$total_importe += (float)$c["importe"];
		$total_interes += (float)$c["interes"];
		$total_total += (float)$c["total"];
		$total_saldo += (float)$c["saldo"];

		$estado = (int)$c["estado"];

		// REGLA DE NEGOCIO DEL PDF:
		// estado 0 (anulado) en este caso realmente significa que ya pagó
		// estado 2 también se considera pagado
		if ($estado === 0 || $estado === 2) {
			$cuotas_pagadas++;
		} else {
			$cuotas_pendientes++;

			if (!empty($c["fechavence"]) && $c["fechavence"] < $fecha_hoy) {
				$cuotas_vencidas++;
			}
		}
	}

	$porcentajeCancelado = count($cuotas) > 0
		? round(($cuotas_pagadas / count($cuotas)) * 100)
		: 0;

	$totalPagado = max(0, (float)$total_total - (float)$total_saldo);
	...
	$data = [
		...
		'porcentajeCancelado' => $porcentajeCancelado,
		'totalPagado' => $totalPagado,
	];

	$html = $this->load->view('creditos/reportes/pdf_cronograma_credito', $data, true);
	...
	$dompdf->render();
	$dompdf->stream('cronograma_credito_'.$credito["codcredito"].'.pdf', array('Attachment' => false));
}
