...
class Listacobranza extends CI_Controller {
	...
	public function index(){
		if ($this->input->is_ajax_request()) {
			if (isset($_SESSION["phuyu_usuario"])) {
				$this->load->view("creditos/listacobranza/index");
			}else{
	...
	function phuyu_imprimirlistapdf(){
		if (isset($_SESSION["phuyu_usuario"])) {
			...
			$info = $this->db->query("select *from caja.v_creditospagos as creditos where creditos.codsucursalcredito=".$_SESSION["phuyu_codsucursal"]." ".$estado." and ".$filtro." creditos.tipo=1 order by creditos.fechamovimiento desc ")->result_array();

			$this->load->library('Pdf2'); $pdf = new Pdf2(); $pdf->AddPage();
			$pdf->pdf_header("LISTA DE COBRANZA GENERAL","");

	        $pdf->SetFont('Arial','B',10); 
	        $pdf->setFillColor(245,245,245); 
	        $pdf->Cell(0,7,"LISTA DE COBRANZA - ".$tipo." DEL ".$this->request->fechadesde.' AL '.$this->request->fechahasta,1,1,'L',1);
