...
class Salidas extends CI_Controller
{
    ...
    public function lista()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $lista = $this->db
                ->query(
                    'select kardex.codmovimientotipo,kardex.codalmacen_ref,kardex.codkardex, kardex.codcomprobantetipo, kardex.seriecomprobante,kardex.nrocomprobante, kardex.fechakardex,round(kardex.importe,2) as importe,kardex.seriecomprobante_ref,kardex.nrocomprobante_ref, personas.razonsocial as cliente,tipos.descripcion as tipomovimiento,comprobantes.descripcion as tipo,kardex.estado from kardex.kardex as kardex inner join public.personas as personas on (kardex.codpersona=personas.codpersona) inner join caja.comprobantetipos as comprobantes on(kardex.codcomprobantetipo_ref=comprobantes.codcomprobantetipo) inner join almacen.movimientotipos as tipos on(kardex.codmovimientotipo=tipos.codmovimientotipo) where ' .
                        $fechas .
                        ' ' .
                        $movimiento .
                        ' codalmacen=' .
                        $_SESSION['phuyu_codalmacen'] .
                        " and (UPPER(personas.razonsocial) like UPPER('%" .
                        $this->request->buscar .
                        "%') or UPPER(tipos.descripcion) like UPPER('%" .
                        $this->request->buscar .
                        "%') or UPPER(kardex.seriecomprobante) like UPPER('%" .
                        $this->request->buscar .
                        "%') or UPPER(kardex.nrocomprobante) like UPPER('%" .
                        $this->request->buscar .
                        "%') ) and tipos.tipo=2 and kardex.codmovimientotipo<>20 order by kardex.codkardex desc offset " .
                        $offset .
                        ' limit ' .
                        $limit,
                )
    ...
    function guardar_original_26012026()
    {
        if ($this->input->is_ajax_request()) {
            if (isset($_SESSION['phuyu_codusuario'])) {
                $this->request = json_decode(file_get_contents('php://input'));
                ...
                $valores = [(int) $this->request->campos->codkardex_ref, (int) $_SESSION['phuyu_codsucursal'], (int) $_SESSION['phuyu_codalmacen'], (int) $this->request->campos->codalmacen_ref, (int) $this->request->campos->codpersona, (int) $_SESSION['phuyu_codusuario'], (int) $this->request->campos->codmovimientotipo, $this->request->campos->fechakardex, $this->request->campos->fechakardex, (int) $this->request->campos->codcomprobantetipo, $this->request->campos->seriecomprobante, (int) $this->request->campos->codcomprobantetipo_ref, $this->request->campos->seriecomprobante_ref, $this->request->campos->nrocomprobante_ref, (float) $this->request->totales->valorventa, (float) $_SESSION['phuyu_igv'], (float) $this->request->totales->igv, (float) $this->request->totales->importe, $this->request->campos->descripcion, $this->request->campos->cliente, $this->request->campos->direccion];
                ...
                $valores = [(int) $_SESSION['phuyu_codsucursal'], (int) $_SESSION['phuyu_codalmacen'], (int) $this->request->campos->codalmacen_ref, (int) $codkardex, (int) $_SESSION['phuyu_codusuario'], (int) $this->request->campos->codmovimientotipo, $this->request->campos->fechakardex, (int) $this->request->campos->codcomprobantetipo, $this->request->campos->seriecomprobante];
                ...
                foreach ($this->request->detalle as $key => $value) {
                    ...
                    $valores = [(int) $codkardex, (int) $this->request->detalle[$key]->codproducto, (int) $this->request->detalle[$key]->codunidad, $item, (float) $this->request->detalle[$key]->cantidad, (float) $this->request->detalle[$key]->precio, (float) $this->request->detalle[$key]->precio, (float) $this->request->detalle[$key]->precio, (float) $this->request->detalle[$key]->precio, '10', (float) $this->request->detalle[$key]->subtotal, (float) $this->request->detalle[$key]->subtotal, (float) $this->request->detalle[$key]->igv, (int) $this->request->detalle[$key]->itemorigen];
                    ...
                    $valores = [(int) $codkardexalmacen, (int) $this->request->detalle[$key]->codproducto, (int) $this->request->detalle[$key]->codunidad, $item, (int) $_SESSION['phuyu_codalmacen'], (int) $_SESSION['phuyu_codsucursal'], (float) $this->request->detalle[$key]->cantidad];
                    ...
                    foreach ($stockconvertido as $k => $val) {
                        ...
                        $stockc = (float) $val['stockactualconvertido'] - $stockc;
    ...
    public function guardar()
    {
        ...
        try {
            // =========================
            // 1) Datos de proveedor/cliente
            // =========================
            $proveedor = $this->db
                ->query(
                    "
						select razonsocial, direccion, documento, d.abreviatura as tipo
            ...
            $codkardex = $this->phuyu_model->phuyu_guardar('kardex.kardex', $campos, $valores, 'true');
            if (!$codkardex) {
                throw new Exception('No se pudo registrar kardex.');
            }

            // =========================
            // 3) Registrar KARDEX ALMACEN (cabecera)
            // =========================
            $campos = ['codsucursal', 'codalmacen', 'codalmacen_ref', 'codkardex', 'codusuario', 'codmovimientotipo', 'fechakardex', 'codcomprobantetipo', 'seriecomprobante'];
            ...
            $codkardexalmacen = $this->phuyu_model->phuyu_guardar('kardex.kardexalmacen', $campos, $valores, 'true');
            if (!$codkardexalmacen) {
                throw new Exception('No se pudo registrar kardexalmacen.');
            }

            $nro_comprobante = $this->Kardex_model->phuyu_kardexcorrelativo($codkardex, $codkardexalmacen, $this->request->campos->codcomprobantetipo, $this->request->campos->seriecomprobante);

            // Para series/comprobante
            $comprobante = $this->request->campos->seriecomprobante . '-' . $nro_comprobante;
            $motivoSerie = !empty($this->request->campos->descripcion) ? $this->request->campos->descripcion : 'SALIDA DE ALMACEN';

            // Cache de factores por producto (evita N queries)
            // factorsCache[codproducto][codunidad] = factor
            $factorsCache = [];
            ...
            // =========================
            // 4) Detalle: Kardexdetalle + Kardexalmacendetalle + Stock + Series
            // =========================
            foreach ($this->request->detalle as $key => $value) {
                $item++;

                $codproducto = (int) $this->request->detalle[$key]->codproducto;
                $codunidad = (int) $this->request->detalle[$key]->codunidad;
                $cantidad = (float) $this->request->detalle[$key]->cantidad;

                // --- 4.1 Guardar kardexdetalle ---
                $campos = ['codkardex', 'codproducto', 'codunidad', 'item', 'cantidad', 'preciobruto', 'preciosinigv', 'preciounitario', 'preciorefunitario', 'codafectacionigv', 'valorventa', 'subtotal', 'igv', 'itemorigen'];

                // OJO: aquí tu código original mandaba precio a varios campos; lo respeto
                $precio = (float) $this->request->detalle[$key]->precio;

                $valores = [(int) $codkardex, $codproducto, $codunidad, $item, (float) $cantidad, (float) $precio, (float) $precio, (float) $precio, (float) $precio, '10', (float) $this->request->detalle[$key]->subtotal, (float) $this->request->detalle[$key]->subtotal, (float) $this->request->detalle[$key]->igv, (int) $this->request->detalle[$key]->itemorigen];

                $estado = $this->phuyu_model->phuyu_guardar('kardex.kardexdetalle', $campos, $valores);
                if (!$estado) {
                    throw new Exception("No se pudo guardar kardexdetalle (item $item).");
                }

                // --- 4.2 Guardar kardexalmacendetalle ---
                $campos = ['codkardexalmacen', 'codproducto', 'codunidad', 'item', 'codalmacen', 'codsucursal', 'cantidad'];
                $valores = [(int) $codkardexalmacen, $codproducto, $codunidad, $item, (int) $_SESSION['phuyu_codalmacen'], (int) $_SESSION['phuyu_codsucursal'], (float) $cantidad];

                $estado = $this->phuyu_model->phuyu_guardar('kardex.kardexalmacendetalle', $campos, $valores);
                if (!$estado) {
                    throw new Exception("No se pudo guardar kardexalmacendetalle (item $item).");
                }
            ...
            ",
                ...
                if (empty($existe)) {
                    throw new Exception("No existe productoubicacion para producto $codproducto unidad $codunidad.");
                }

                // --- 4.4 Validación de stock antes de descontar ---
                if ($_SESSION['phuyu_stockalmacen'] == 1 && (float) $existe[0]['stockactualconvertido'] < $cantidad) {
                    $informacion['success'] = false;
                    $informacion['stock'][$key] = $existe[0]['stockactualconvertido'];
                    $informacion['producto'][$key] = $this->request->detalle[$key]->producto ?? '';
                    $informacion['unidad'][$key] = $this->request->detalle[$key]->unidad ?? '';
                ...
                // --- 4.5 Validación de serie ANTES de descontar ---
                $controlarseries = isset($this->request->detalle[$key]->controlarseries) ? (int) $this->request->detalle[$key]->controlarseries : 0;

                $serieId = null;

                if ($controlarseries === 1) {
                    // cantidad debe ser 1
                    if ($cantidad != 1.0) {
                        $informacion['success'] = false;
                        $informacion['serie'][$key] = 'Producto por serie debe tener cantidad 1';
                        break;
                    }

                    $serie = $this->request->detalle[$key]->serie_seleccionada ?? null;
                    if (!$serie || !isset($serie->id_serie)) {
                        $informacion['success'] = false;
                        $informacion['serie'][$key] = 'Serie no seleccionada';
                        break;
                    }

                    $serieId = (int) $serie->id_serie;

                    $serieRow = $this->db
                        ->get_where('almacen.series', [
                            'id_serie' => $serieId,
                    ...
                    if (empty($serieRow)) {
                        $informacion['success'] = false;
                        $informacion['serie'][$key] = 'Serie no disponible en almacén';
                        break;
                    }
                }

                // --- 4.6 Descontar stockactual (UPDATE atómico) ---
                $estado = $this->db->query(
                    "
                update almacen.productoubicacion
                set stockactual = round(stockactual - ?, 3)
                where codalmacen = ?
                  and codproducto = ?
                  and codunidad   = ?
            ",
                    [(float) $cantidad, (int) $_SESSION['phuyu_codalmacen'], $codproducto, $codunidad],
                );

                if (!$estado) {
                    throw new Exception("No se pudo actualizar stockactual (item $item).");
                }

                // --- 4.7 Descontar stockactualconvertido (todas las unidades del producto) ---
                // cache de factores por producto
                if (!isset($factorsCache[$codproducto])) {
                    $rows = $this->db
                        ->query(
                            "
                    select codunidad, factor
                ...
                ",
                    ...
                    $factorsCache[$codproducto] = [];
                    foreach ($rows as $r) {
                        $factorsCache[$codproducto][(int) $r['codunidad']] = (float) $r['factor'];
                    }
                }

                if (empty($factorsCache[$codproducto][$codunidad])) {
                    throw new Exception("No se encontró factor para producto $codproducto unidad $codunidad.");
                }

                // leer todas las filas de ubicación del producto (bloqueo)
                $stockconvertido = $this->db
                    ->query(
                        "
                select codunidad
                from almacen.productoubicacion
            ...
            ",
                ...
                $factorBase = (float) $factorsCache[$codproducto][$codunidad];

                foreach ($stockconvertido as $sc) {
                    $u = (int) $sc['codunidad'];

                    if (empty($factorsCache[$codproducto][$u])) {
                        throw new Exception("No se encontró factor para producto $codproducto unidad $u.");
                    }

                    $factorDestino = (float) $factorsCache[$codproducto][$u];

                    // cantidad convertida a unidad u
                    $descuentoConvertido = ((float) $cantidad * $factorBase) / $factorDestino;

                    $estado = $this->db->query(
                        "
                    update almacen.productoubicacion
                    set stockactualconvertido = round(stockactualconvertido - ?, 3)
                    where codalmacen = ?
                      and codproducto = ?
                      and codunidad   = ?
                ",
                        [(float) $descuentoConvertido, (int) $_SESSION['phuyu_codalmacen'], $codproducto, $u],
                    );

                    if (!$estado) {
                        throw new Exception("No se pudo actualizar stockactualconvertido (producto $codproducto, unidad $u).");
                    }
                }

                // --- 4.8 Marcar serie como salida (si aplica) ---
                if ($controlarseries === 1 && $serieId) {
                    $estado = $this->db->where('id_serie', $serieId)->update('almacen.series', [
                        'estado'            => 'DADO_BAJA',
                        'fecha_egreso'      => date('Y-m-d H:i:s'),
                        'codkardex_egreso'  => (int) $codkardex,
                        'motivo'            => $motivoSerie,
                    ...
                    if (!$estado) {
                        throw new Exception("No se pudo actualizar serie (id_serie $serieId).");
                    }
                }
            }

            // =========================
            // 5) Si hubo error lógico (stock/serie), rollback y salida
            // =========================
            if (!$informacion['success']) {
                $this->db->trans_rollback();
                $data['estado'] = 2;
    ...
    function editar_guardar()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $valores = [$this->request->codmovimientotipo, $this->request->fechacomprobante, $this->request->fechakardex, $this->request->descripcion, $this->request->cliente, $this->request->direccion];
    ...
    function eliminar()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $valores = [(int) $this->request->codregistro, (int) $_SESSION['phuyu_codsucursal'], (int) $_SESSION['phuyu_codusuario'], date('Y-m-d'), 'SALIDA DE ALMACEN ANULADO'];
            ...
            if (count($kardexalmacen) > 0) {
                ...
                $valores = [(int) $kardexalmacen[0]['codkardexalmacen'], (int) $_SESSION['phuyu_codsucursal'], (int) $_SESSION['phuyu_codusuario'], date('Y-m-d'), 'SALIDA DE ALMACEN ANULADO'];
    ...
    function formato($formato)
    {
        if ($this->input->is_ajax_request()) {
            $campos = ['formato'];
            $valores = [$formato];
            $f = ['codsucursal', 'codcomprobantetipo'];
            $v = [$_SESSION['phuyu_codsucursal'], 4];
    ...
    function guardar_operacionstock()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $subtotal = (float) $this->request->preciocosto * (float) $this->request->cantidad;
            $valores = [(int) $_SESSION['phuyu_codsucursal'], (int) $_SESSION['phuyu_codalmacen'], 1, (int) $_SESSION['phuyu_codusuario'], 21, $this->request->fechakardex, $this->request->fechakardex, $codcomprobantetipo, $seriecomprobante, 0, 'SIN', '00000000', (float) round($subtotal, 2), (float) $_SESSION['phuyu_igv'], (float) 0, (float) round($subtotal, 2), 'SALIDA POR AJUSTES EN VENTA'];
            $codkardex = $this->phuyu_model->phuyu_guardar('kardex.kardex', $campos, $valores, 'true');
            $codkardex_ref = $codkardex;

            // REGISTRO KARDEX ALMACEN //
            $campos = ['codsucursal', 'codalmacen', 'codkardex', 'codusuario', 'codmovimientotipo', 'fechakardex', 'codcomprobantetipo', 'seriecomprobante'];
            $valores = [(int) $_SESSION['phuyu_codsucursal'], (int) $_SESSION['phuyu_codalmacen'], (int) $codkardex, (int) $_SESSION['phuyu_codusuario'], 21, $this->request->fechakardex, (int) $codcomprobantetipo, $seriecomprobante];
            $codkardexalmacen = $this->phuyu_model->phuyu_guardar('kardex.kardexalmacen', $campos, $valores, 'true');
            ...
            $valores = [(int) $codkardex, (int) $this->request->codproducto, (int) $this->request->codunidad, 1, (float) $this->request->cantidad, (float) $this->request->preciocosto, (float) $this->request->preciocosto, (float) $this->request->preciocosto, (float) $this->request->preciocosto, 20, (float) round($subtotal, 2), (float) round($subtotal, 2)];
            $estado = $this->phuyu_model->phuyu_guardar('kardex.kardexdetalle', $campos, $valores);

            $campos = ['codkardexalmacen', 'codproducto', 'codunidad', 'item', 'codalmacen', 'codsucursal', 'cantidad'];
            $valores = [(int) $codkardexalmacen, (int) $this->request->codproducto, (int) $this->request->codunidad, 1, (int) $_SESSION['phuyu_codalmacen'], (int) $_SESSION['phuyu_codsucursal'], (float) $this->request->cantidad];
            $estado = $this->phuyu_model->phuyu_guardar('kardex.kardexalmacendetalle', $campos, $valores);
            ...
            $subtotal = (float) $preciocosto * (float) $cantidad;
            $codunidad = $this->request->codunidad_ingreso;

            $valores = [(int) $codkardex_ref, (int) $_SESSION['phuyu_codsucursal'], (int) $_SESSION['phuyu_codalmacen'], 1, (int) $_SESSION['phuyu_codusuario'], 3, $this->request->fechakardex, $this->request->fechakardex, $codcomprobantetipo, $seriecomprobante, 0, 'SIN', '00000000', (float) round($subtotal, 2), (float) $_SESSION['phuyu_igv'], (float) 0, (float) round($subtotal, 2), 'INGRESO POR AJUSTES EN VENTA'];
            $codkardex = $this->phuyu_model->phuyu_guardar('kardex.kardex', $campos, $valores, 'true');

            $campos = ['codkardex_ref'];
            $valores = [$codkardex];
            $estado = $this->phuyu_model->phuyu_editar('kardex.kardex', $campos, $valores, 'codkardex', $codkardex_ref);

            // REGISTRO KARDEX ALMACEN //
            $campos = ['codsucursal', 'codalmacen', 'codkardex', 'codusuario', 'codmovimientotipo', 'fechakardex', 'codcomprobantetipo', 'seriecomprobante'];
            $valores = [(int) $_SESSION['phuyu_codsucursal'], (int) $_SESSION['phuyu_codalmacen'], (int) $codkardex, (int) $_SESSION['phuyu_codusuario'], 3, $this->request->fechakardex, (int) $codcomprobantetipo, $seriecomprobante];
            $codkardexalmacen = $this->phuyu_model->phuyu_guardar('kardex.kardexalmacen', $campos, $valores, 'true');
            ...
            $campos = ['codkardex', 'codproducto', 'codunidad', 'item', 'cantidad', 'preciobruto', 'preciosinigv', 'preciounitario', 'preciorefunitario', 'codafectacionigv', 'valorventa', 'subtotal'];
            $valores = [(int) $codkardex, (int) $this->request->codproducto, (int) $codunidad, 1, (float) round($cantidad, 3), (float) round($preciocosto, 2), (float) round($preciocosto, 2), (float) round($preciocosto, 2), (float) round($preciocosto, 2), 20, (float) round($subtotal, 2), (float) round($subtotal, 2)];
            $estado = $this->phuyu_model->phuyu_guardar('kardex.kardexdetalle', $campos, $valores);

            $campos = ['codkardexalmacen', 'codproducto', 'codunidad', 'item', 'codalmacen', 'codsucursal', 'cantidad'];
            $valores = [(int) $codkardexalmacen, (int) $this->request->codproducto, (int) $codunidad, 1, (int) $_SESSION['phuyu_codalmacen'], (int) $_SESSION['phuyu_codsucursal'], (float) $cantidad];
            $estado = $this->phuyu_model->phuyu_guardar('kardex.kardexalmacendetalle', $campos, $valores);
    ...
    function guardar_prestamo()
    {
        if ($this->input->is_ajax_request()) {
            if (isset($_SESSION['phuyu_codusuario'])) {
                ...
                $valores = [(int) $this->request->kardex_ref, (int) $_SESSION['phuyu_codsucursal'], (int) $_SESSION['phuyu_codalmacen'], (int) $info[0]['codalmacen'], $this->request->codpersona, (int) $_SESSION['phuyu_codusuario'], 29, date('Y-m-d'), date('Y-m-d'), 4, $series[0]['seriecomprobante'], (int) $info[0]['codcomprobantetipo_ref'], $info[0]['seriecomprobante_ref'], $info[0]['nrocomprobante_ref'], 'SALIDA POR DEVOLUCION DE PRESTAMOS RECIBIDOS'];
                $codkardex = $this->phuyu_model->phuyu_guardar('kardex.kardex', $campos, $valores, 'true');

                // REGISTRO KARDEX ALMACEN //
                $campos = ['codsucursal', 'codalmacen', 'codalmacen_ref', 'codkardex', 'codusuario', 'codmovimientotipo', 'fechakardex', 'codcomprobantetipo', 'seriecomprobante'];
                $valores = [(int) $_SESSION['phuyu_codsucursal'], (int) $_SESSION['phuyu_codalmacen'], (int) $info[0]['codalmacen'], (int) $codkardex, (int) $_SESSION['phuyu_codusuario'], 29, date('Y-m-d'), 4, $series[0]['seriecomprobante']];
                $codkardexalmacen = $this->phuyu_model->phuyu_guardar('kardex.kardexalmacen', $campos, $valores, 'true');
                ...
                foreach ($this->request->detalle as $key => $value) {
                    $item = $item + 1;
                    $subtotal = $subtotal + (float) $this->request->detalle[$key]->subtotal;
                    $igv = $igv + (float) $this->request->detalle[$key]->igv;
                    $total = $total + (float) $this->request->detalle[$key]->subtotal;

                    $campos = ['codkardex', 'codproducto', 'codunidad', 'item', 'cantidad', 'preciobruto', 'preciosinigv', 'preciounitario', 'preciorefunitario', 'codafectacionigv', 'valorventa', 'subtotal', 'igv', 'itemorigen'];
                    $valores = [(int) $codkardex, (int) $this->request->detalle[$key]->codproducto, (int) $this->request->detalle[$key]->codunidad, $item, (float) $this->request->detalle[$key]->cantidad, (float) $this->request->detalle[$key]->preciounitario, (float) $this->request->detalle[$key]->preciounitario, (float) $this->request->detalle[$key]->preciounitario, (float) $this->request->detalle[$key]->preciounitario, $this->request->detalle[$key]->codafectacionigv, (float) $this->request->detalle[$key]->subtotal, (float) $this->request->detalle[$key]->subtotal, (float) $this->request->detalle[$key]->igv, (int) $this->request->detalle[$key]->itemorigen];
                    $estado = $this->phuyu_model->phuyu_guardar('kardex.kardexdetalle', $campos, $valores);

                    $campos = ['codkardexalmacen', 'codproducto', 'codunidad', 'codalmacen', 'item', 'codsucursal', 'cantidad'];
                    $valores = [(int) $codkardexalmacen, (int) $this->request->detalle[$key]->codproducto, (int) $this->request->detalle[$key]->codunidad, (int) $_SESSION['phuyu_codalmacen'], $item, (int) $_SESSION['phuyu_codsucursal'], (float) $this->request->detalle[$key]->cantidad];
                    $estado = $this->phuyu_model->phuyu_guardar('kardex.kardexalmacendetalle', $campos, $valores);
                    ...
                    $campos = ['cantidaddevuelta'];
                    $valores = [(float) $cantidaddevuelta[0]['cantidaddevuelta'] + (float) $cantidaddevueltaconvertida];
                ...
                $campos = ['valorventa', 'porcigv', 'igv', 'importe'];
                $valores = [$subtotal, $_SESSION['phuyu_igv'], $igv, $total];
