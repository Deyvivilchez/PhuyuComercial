...
class Prestamosdevolucion extends CI_Controller {
	...
	public function index(){
		if ($this->input->is_ajax_request()) {
			if (isset($_SESSION["phuyu_usuario"])) {
				$this->load->view("almacen/prestamosdevolver/index");
			}else{
	...
	public function lista(){
		if ($this->input->is_ajax_request()) {
			...
			$lista = $this->db->query("SELECT k.codkardex,k.codsucursal,k.codalmacen,(sum(kd.cantidad) - sum(kd.cantidaddevuelta)) As cantidaddevuelta,p.razonsocial as cliente,k.direccion,k.fechakardex,k.hora,k.seriecomprobante,k.nrocomprobante,p.documento,round(k.valorventa,2) as valorventa,round(k.importe,2) as importe,k.codempleado,k.codpersona,k.condicionpago,k.igv,k.codcomprobantetipo,k.codmovimientotipo FROM kardex.kardex k JOIN kardex.kardexdetalle kd ON k.codkardex = kd.codkardex JOIN public.personas as p on (k.codpersona=p.codpersona) WHERE k.codmovimientotipo = 7 and k.codsucursal=".$_SESSION["phuyu_codsucursal"]." and k.estado=1 GROUP BY k.codkardex, k.codsucursal,k.codpersona, k.valorventa, k.codalmacen,p.razonsocial,k.fechakardex,k.hora,k.seriecomprobante,k.nrocomprobante,p.documento ".$where." order by k.fechakardex asc ")->result_array();

			$total = $this->db->query("select count(*) as total FROM kardex.kardex k JOIN kardex.kardexdetalle kd ON k.codkardex = kd.codkardex WHERE k.codmovimientotipo = 7 and k.codsucursal=".$_SESSION["phuyu_codsucursal"]." and k.estado=1 GROUP BY k.codkardex, k.codsucursal,k.codpersona, k.valorventa, k.codalmacen,k.cliente,k.fechakardex,k.hora,k.seriecomprobante,k.nrocomprobante ".$where." ")->result_array();
	...
	public function nuevo($codkardex){
		if ($this->input->is_ajax_request()) {
			...
			$this->load->view("almacen/prestamosdevolver/nuevo",compact("info"));
		}else{
			$this->load->view("phuyu/404");
		}
	}
	...
	function historial($codkardex){
		if ($this->input->is_ajax_request()) {
			...
			$this->load->view("almacen/prestamosdevolver/historial",compact("info"));
		}else{
			$this->load->view("phuyu/404");
		}
	}
	...
	function editar(){
		if ($this->input->is_ajax_request()) {
			...
			$this->load->view("almacen/prestamosdevolver/editar",compact("info"));
		}
	}
