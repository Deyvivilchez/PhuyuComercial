...
class Clientes extends CI_Controller
{
	...
	public function index()
	{
		...
		// Pasar datos a la vista unificada
		$data = compact('tipodocumentos', 'departamentos');
		$this->load->view('ventas/clientes/index', $data);
	}
	...
	public function lista()
	{
		if ($this->input->is_ajax_request()) {
			...
			$lista = $this->db
				->query(
					"select personas.* from public.socios as socios inner join public.personas as personas on (socios.codpersona=personas.codpersona)
   where (UPPER(personas.documento) like UPPER('%" .
						$this->request->buscar .
						"%') or UPPER(personas.razonsocial) like UPPER('%" .
						$this->request->buscar .
						"%')
   or UPPER(personas.nombrecomercial) like UPPER('%" .
						$this->request->buscar .
						"%') ) and (socios.codsociotipo=1 or socios.codsociotipo=3) and socios.estado=1
   AND personas.codpersona>2 order by personas.codpersona desc offset " .
						$offset .
						' limit ' .
						$limit,
				)
	...
	public static function sumarDiasNaturales($fecha, $dias)
	{
		...
		$sd = $dias;
		while ($sd > 0) {
			if ($sd <= date('t', mktime(0, 0, 0, $dateArray[1], 1, $dateArray[0])) - $dateArray[2]) {
				$dateArray[2] = $dateArray[2] + $sd;
				$sd = 0;
			} else {
				$sd = $sd - (date('t', mktime(0, 0, 0, $dateArray[1], 1, $dateArray[0])) - $dateArray[2]);
				$dateArray[2] = 0;
				if ($dateArray[1] < 12) {
					$dateArray[1]++;
				} else {
		...
		if ($this->input->is_ajax_request()) {
			...
			$mail->Body =
						...
						<table border="0" cellpadding="0" cellspacing="0" width="440px" style=color:rgb(0,0,0);font-family:"Times New Roman";font-size:medium">
							<tbody>
								<tr ><td colspan="2" style="color:#515559;font-family:Arial,Helvetica,sans-serif;font-size:14px;font-weight:bold;text-align:center" ><strong>' .
						...
						<td align="left" valign="top" style="padding:0px 9px 0px 0px">
							...
							<strong style="color:rgb(166,174,172);font-family:Arial,Helvetica,sans-serif;font-size:12px">
							<font color="#384845">' .
									$venta[0]['seriecomprobante'] .
									'-' .
									$venta[0]['nrocomprobante'] .
									'</font>
							</strong><br>
							...
							<strong style="color:rgb(166,174,172);font-family:Arial,Helvetica,sans-serif;font-size:12px">
							<font color="#384845">' .
									number_format($venta[0]['importe'], 2) .
									'</font>
							</strong><br>
