...
class Proformas extends CI_Controller {
	...
	public function index(){
		if ($this->input->is_ajax_request()) {
			if (isset($_SESSION["phuyu_codusuario"])) {
				...
				$this->load->view("ventas/proformas/index",compact("almacen"));
			}else{
	...
	public function buscar(){
        if ($this->input->is_ajax_request()) {
			if (isset($_SESSION["phuyu_codusuario"])) {
				$this->load->view("ventas/proformas/buscar");
			}else{
				$this->load->view("phuyu/505");
			}
		}else{
	...
	public function nuevo(){
		if ($this->input->is_ajax_request()) {
			if (isset( $_SESSION["phuyu_codusuario"]) ) {
				...
				$this->load->view("ventas/proformas/nuevo",compact("comprobantes","comprobantesreferencia","tipopagos","vendedores","sucursal","sucursalreferencia","centrocostos","afectacionigv"));
			}else{
				$this->load->view("phuyu/505");
			}
		}else{
			...
			if (isset( $_SESSION["phuyu_codusuario"]) ) {
				...
				if($this->request->campos->codproforma == 0){
					...
					foreach ($this->request->detalle as $key => $value) { 
						...
						$campos = ["codproforma","codproducto","codunidad","item","cantidad","preciounitario","igv","preciosinigv","preciobruto","valorventa","preciorefunitario","codafectacionigv","subtotal","descripcion"];
						$valores =[
							(int)$codproforma,
				    ...
				    if (count($this->request->detalle) > 0) {
                        ...
                        foreach ($this->request->detalle as $key => $value) { 
							...
							$campos = ["codproforma","codproducto","codunidad","item","cantidad","preciounitario","igv","preciobruto","preciosinigv","valorventa","preciorefunitario","codafectacionigv","subtotal","descripcion"];
							$valores =[
								(int)$codproforma,
								(int)$this->request->detalle[$key]->codproducto,
								(int)$this->request->detalle[$key]->codunidad, $item,
	...
	function editar(){
		if ($this->input->is_ajax_request()) {
			...
			if(count($total) > 0){
				...
				$afectacionigv = $this->db->query("select *from afectacionigv where estado = 1")->result_array();
	            $this->load->view("ventas/proformas/editar",compact("info","detalle","vendedores","tipopagos","comprobantes","afectacionigv"));
			}else{
			    echo 1;
			}
		...
		if ($this->input->is_ajax_request()) {
			if (isset( $_SESSION["phuyu_codusuario"]) ) {
				...
				if($info[0]["condicionpago"]==2){
					$credito = $this->db->query("select *from kardex.creditosproformas where codproforma=".$info[0]["codproforma"])->result_array();
