...
class Ventas extends CI_Controller {
	...
	function eliminar(){
		if ($this->input->is_ajax_request()) {
			...
			foreach ($info as $key => $value) {
				...
				$campos = ["stockactual","ventarecogo"]; $valores = [(double)$stock, (double)$existe[0]["ventarecogo"] - $cantidad_recogo];
				...
				foreach ($stockconvertido as $k => $val) {
                    ...
                    $campos = ["stockactualconvertido","ventarecogoconvertido"]; $valores = [(double)$stockc,(double)$val["ventarecogoconvertido"] - (double)$stockrecoger];
					$f = ["codalmacen","codproducto","codunidad"];
	...
	function formato($formato){
		if ($this->input->is_ajax_request()) {
			...
			$f = ["codsucursal"]; $v = [$_SESSION["phuyu_codsucursal"]];
