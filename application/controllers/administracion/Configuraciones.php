...
function guardar(){
    if ($this->input->is_ajax_request()) {
        ...
        try {
            ...
            if ($codubigeo === null || $codubigeo === '') {
                echo json_encode([
                    "estado" => 0,
                    "paso" => "validando codubigeo",
                    "error" => "codubigeo viene vacío",
                    "post" => $_POST
                ]);
                return;
            }

            $query = $this->db->query("select * from public.ubigeo where codubigeo=".$codubigeo);

            if (!$query) {
                echo json_encode([
                    "estado" => 0,
                    "paso" => "consultando ubigeo",
                    "error" => $this->db->error()
                ]);
                return;
            }

            $ubigeo = $query->result_array();

            if (count($ubigeo) == 0) {
                echo json_encode([
                    "estado" => 0,
                    "paso" => "buscando ubigeo",
                    "error" => "No existe codubigeo: ".$codubigeo
                ]);
                return;
            }
            ...
            $valores = [
                ...
                $this->input->post("nombrecomercial"),
            ...
            $estado = $this->phuyu_model->phuyu_editar(
                ...
                $valores,
                "codpersona",
                $this->input->post("codpersona")
            );

            if (!$estado) {
                echo json_encode([
                    "estado" => 0,
                    "paso" => "actualizando personas",
                    "error" => $this->db->error()
                ]);
                return;
            }
            ...
            // EMPRESAS
            $campos = [
                "igvsunat","icbpersunat","iscsunat","slogan","itemrepetircomprobante",
                "claveseguridad","publicidad","agradecimiento","ubigeo","departamento",
                "provincia","distrito","leyendapamazonia","codleyendapamazonia",
                "leyendasamazonia","codleyendasamazonia","urlconsultacomprobantes"
            ];

            $valores = [
                (double)$this->input->post("igvsunat"),
                (double)$this->input->post("icbpersunat"),
                (double)$this->input->post("iscsunat"),
                $this->input->post("slogan"),
                $this->input->post("itemrepetircomprobante"),
                $this->input->post("claveseguridad"),
                $this->input->post("publicidad"),
                $this->input->post("agradecimiento"),
                $ubigeo[0]["ubidepartamento"].$ubigeo[0]["ubiprovincia"].$ubigeo[0]["ubidistrito"],
                $ubigeo[0]["departamento"],
                $ubigeo[0]["provincia"],
                $ubigeo[0]["distrito"],
                $this->input->post("leyendapamazonia"),
                $this->input->post("codleyendapamazonia"),
                $this->input->post("leyendasamazonia"),
                $this->input->post("codleyendasamazonia"),
                $this->input->post("urlconsultacomprobantes")
            ];
            ...
            if (isset($_FILES["logo"]) && $_FILES["logo"]["name"] != "") {
                ...
                $ruta = "./public/img/empresa/".$file;

                if (!move_uploaded_file($_FILES["logo"]["tmp_name"], $ruta)) {
                    echo json_encode([
                        "estado" => 0,
                        "paso" => "subiendo logo",
                        "error_file" => $_FILES["logo"]["error"],
                        "ruta" => $ruta
                    ]);
                    return;
                }
                ...
                $data = ["foto" => $file];
                $this->db->where("codpersona",$this->input->post("codpersona"));
                $estado = $this->db->update("public.personas",$data);
            }

            // AUSPICIADOR
            if (isset($_FILES["auspiciador"]) && $_FILES["auspiciador"]["name"] != "") {

                $file = "auspiciador_".substr($_FILES["auspiciador"]["name"], -5);
                $ruta = "./public/img/empresa/".$file;

                if (!move_uploaded_file($_FILES["auspiciador"]["tmp_name"], $ruta)) {
                    echo json_encode([
                        "estado" => 0,
                        "paso" => "subiendo auspiciador",
                        "error_file" => $_FILES["auspiciador"]["error"],
                        "ruta" => $ruta
                    ]);
                    return;
                }

                $data = ["logoauspiciador" => $file];
                $this->db->where("codempresa",$this->input->post("codempresa"));
                $estado = $this->db->update("public.empresas",$data);
            }

            // SESSION
            $_SESSION["phuyu_ruc"] = $this->input->post("documento");
            $_SESSION["phuyu_empresa"] = ($this->input->post("nombrecomercial")=='')
                ? $this->input->post("razonsocial")
                : $this->input->post("nombrecomercial");

            $_SESSION["phuyu_igv"] = (double)$this->input->post("igvsunat");
            $_SESSION["phuyu_icbper"] = (double)$this->input->post("icbpersunat");
            $_SESSION["phuyu_itemrepetir"] = $this->input->post("itemrepetircomprobante");
