...
class Pedidos extends CI_Controller {
	...
	public function index(){
		if ($this->input->is_ajax_request()) {
			if (isset($_SESSION["phuyu_codusuario"])) {
				...
				$formato = $this->db->query("select formato from caja.comprobantes where codcomprobantetipo=30 AND codsucursal = ".$_SESSION["phuyu_codsucursal"])->result_array();
				if (count($formato)==0) {
				...
				$comprobante_almacen = $this->db->query("select count(*) as cantidad from caja.comprobantes where codcomprobantetipo=30 and codsucursal=".$_SESSION["phuyu_codsucursal"]." and estado=1")->result_array();
				$almacen = $comprobante_almacen[0]["cantidad"];
				$this->load->view("compras/pedidos/index",compact("almacen"));
			}else{
	...
	public function lista(){
		if ($this->input->is_ajax_request()) {
			...
			$lista = $this->db->query("select pedidos.hora,personas.documento,pedidos.cliente,pedidos.codpedido, pedidos.codcomprobantetipo, pedidos.seriecomprobante,pedidos.condicionpago, pedidos.nrocomprobante, pedidos.fechapedido,round(pedidos.importe,2) as importe,pedidos.estado, comprobantes.descripcion as tipo,pedidos.codkardex,pedidos.estadoproceso from kardex.pedidos inner join public.personas as personas on (pedidos.codpersona=personas.codpersona) inner join caja.comprobantetipos as comprobantes on(pedidos.codcomprobantetipo=comprobantes.codcomprobantetipo) where ".$fechas." (UPPER(personas.documento) like UPPER('%".$this->request->buscar."%') or UPPER(personas.razonsocial) like UPPER('%".$this->request->buscar."%') or UPPER(pedidos.cliente) like UPPER('%".$this->request->buscar."%') or UPPER(comprobantes.descripcion) like UPPER('%".$this->request->buscar."%') or UPPER(pedidos.seriecomprobante) like UPPER('%".$this->request->buscar."%') or UPPER(pedidos.nrocomprobante) like UPPER('%".$this->request->buscar."%') ) and pedidos.codsucursal=".$_SESSION["phuyu_codsucursal"]." AND tipopedido = 1 order by pedidos.fechapedido desc,pedidos.hora desc offset ".$offset." limit ".$limit)->result_array();
			...
			$total = $this->db->query("select count(*) as total from kardex.pedidos inner join public.personas as personas on (pedidos.codpersona=personas.codpersona) inner join caja.comprobantetipos as comprobantes on(pedidos.codcomprobantetipo=comprobantes.codcomprobantetipo) where ".$fechas." (UPPER(personas.documento) like UPPER('%".$this->request->buscar."%') or UPPER(personas.razonsocial) like UPPER('%".$this->request->buscar."%') or UPPER(pedidos.cliente) like UPPER('%".$this->request->buscar."%') or UPPER(comprobantes.descripcion) like UPPER('%".$this->request->buscar."%') or UPPER(pedidos.seriecomprobante) like UPPER('%".$this->request->buscar."%') or UPPER(pedidos.nrocomprobante) like UPPER('%".$this->request->buscar."%') ) and pedidos.codsucursal=".$_SESSION["phuyu_codsucursal"]." AND tipopedido = 1")->result_array();
	...
	public function buscar_lista(){
		if ($this->input->is_ajax_request()) {
			...
			$total = $this->db->query("select count(*) as total FROM kardex.pedidos k JOIN kardex.pedidosdetalle kd ON k.codpedido = kd.codpedido JOIN public.personas as p on (k.codpersona=p.codpersona) WHERE (UPPER(p.documento) ilike UPPER('%".$this->request->buscar."%') or UPPER(p.razonsocial) like UPPER('%".$this->request->buscar."%') or UPPER(k.cliente) like UPPER('%".$this->request->buscar."%') or UPPER(k.seriecomprobante) like UPPER('%".$this->request->buscar."%') or UPPER(k.nrocomprobante) like UPPER('%".$this->request->buscar."%') ) and k.codsucursal=".$_SESSION["phuyu_codsucursal"]." AND estadoproceso=0 and k.estado=1 and tipopedido = 1 HAVING sum(kd.cantidadcomprobante) < sum(kd.cantidad)")->result_array();
	...
	public function buscar(){
        if ($this->input->is_ajax_request()) {
			if (isset($_SESSION["phuyu_codusuario"])) {
				$this->load->view("compras/pedidos/buscar");
			}else{
				$this->load->view("phuyu/505");
			}
		}else{
	...
	public function nuevo(){
		if ($this->input->is_ajax_request()) {
			if (isset( $_SESSION["phuyu_codusuario"]) ) {
				...
				$comprobantes = $this->db->query("select distinct(ct.codcomprobantetipo) as codigo, ct.*, c.seriecomprobante from caja.comprobantetipos as ct inner join caja.comprobantes as c on(ct.codcomprobantetipo=c.codcomprobantetipo) where c.codsucursal=".$_SESSION["phuyu_codsucursal"]." and c.codcomprobantetipo=30 and c.estado=1")->result_array();
				...
				$sucursal = $this->db->query("select coalesce(codcomprobantetipo,30) as codcomprobantetipo, seriecomprobante from public.sucursales where codsucursal=".$_SESSION["phuyu_codsucursal"])->result_array();
				...
				$this->load->view("compras/pedidos/nuevo",compact("comprobantes","comprobantesreferencia","tipopagos","vendedores","sucursal","sucursalreferencia","centrocostos"));
			}else{
				$this->load->view("phuyu/505");
			}
		}else{
	...
	public function historial($codpersona){
		if ($this->input->is_ajax_request()) {
			if (isset( $_SESSION["phuyu_codusuario"]) ) {
				...
				$this->load->view("compras/pedidos/historial",compact("persona"));
			}else{
				$this->load->view("phuyu/505");
			}
		}else{
	...
	function atender($codpedido){
		if ($this->input->is_ajax_request()) {
			if (isset( $_SESSION["phuyu_codusuario"]) ) {
				...
				$this->load->view("compras/pedidos/atender",compact("info"));
			}else{
				$this->load->view("phuyu/505");
			}
		}else{
	...
	function ver($codpedido){
		if ($this->input->is_ajax_request()) {
			if (isset($_SESSION["phuyu_codusuario"])){
				$info = $this->db->query("select pedido.*,p.*,comprobantes.descripcion as tipo,(CASE WHEN condicionpago = 1 THEN 'CONTADO' ELSE 'CREDITO' END) AS pago,(CASE WHEN estadoproceso = 0 THEN 'PENDIENTE' ELSE 'ATENDIDO' END) AS proceso from kardex.pedidos as pedido inner join caja.comprobantetipos as comprobantes on(pedido.codcomprobantetipo=comprobantes.codcomprobantetipo) INNER JOIN public.personas as p on (pedido.codpersona=p.codpersona) where pedido.codpedido=".$codpedido)->result_array();
				...
				$this->load->view("compras/pedidos/ver",compact("info","detalle","totales")); 
			}else{
	            $this->load->view("phuyu/505");
	        }
	    }else{
	...
	function editar(){
		if ($this->input->is_ajax_request()) {
			...
			$total = $this->db->query("select count(*) as total FROM kardex.pedidos k JOIN kardex.pedidosdetalle kd ON k.codpedido = kd.codpedido JOIN public.personas as p on (k.codpersona=p.codpersona) WHERE k.codsucursal=".$_SESSION["phuyu_codsucursal"]." and k.estado=1 and tipopedido = 1 AND estadoproceso = 0 AND k.codpedido =".$this->request->codregistro." HAVING sum(kd.cantidadcomprobante)=0")->result_array();
			...
			if(count($total) > 0){
	            ...
	            $this->load->view("compras/pedidos/editar",compact("info","detalle","comprobantesreferencia","vendedores","tipopagos"));
			}else{
			    echo 1;
			}
	...
	function cobrar_pedido(){
		if ($this->input->is_ajax_request()) {
			if (isset( $_SESSION["phuyu_codusuario"]) ) {
				...
				$codmovimiento = $this->Caja_model->phuyu_movimientos($codkardex, 1, 1, $this->request->totales->importe, $this->request->campos);
		...
		if ($this->input->is_ajax_request()) {
			if (isset( $_SESSION["phuyu_codusuario"]) ) {
				...
				if($info[0]["condicionpago"]==2){
					$credito = $this->db->query("select *from kardex.creditospedidos where codpedido=".$info[0]["codpedido"]." AND estado = 1")->result_array();
