...
class Formato extends CI_Controller
{
    ...
    private function phuyu_pdf_image_src($path)
    {
        ...
        $cacheDir = APPPATH . 'cache/pdf_images/';
        if (!is_dir($cacheDir)) {
        ...
        $cachePath = $cacheDir . md5(realpath($path) . '|no-interlace|' . filemtime($path)) . '.png';
 ...
 $html .= '</tr>';
    ...
    //         $html .=
    //             '<p style="text-align:left !important;font-size:9px"><b>' .
    //             $value['banco'] .
    //             '</b> <br>CC: &nbsp;' .
    //             $value['nroctacte'] .
    //             '<br> CCI: ' .$value['descripcion'] .'</p><br>';
    //     }
    ...
    public function a4($codkardex)
{
    ...
    if (empty($venta)) {
        show_error("No se encontró el comprobante solicitado.", 404);
        return;
    }
    ...
    if ((int)$venta['condicionpago'] === 2) {
        $credito = $this->db->query("
            ...
            WHERE codkardex = " . (int)$venta['codkardex'] . "
            LIMIT 1
        ")->row_array();
    ...
    $formato = $this->db->query("
        SELECT nombrecomercial, logo, slogan, publicidad, agradecimiento, tipoconleyendaamazonia, impresionlogo
    ...
    if (empty($formato)) {
        $formato = [
            ...
            'publicidad' => '',
            'agradecimiento' => '',
            'tipoconleyendaamazonia' => 1,
            'impresionlogo' => 1,
        ];
    }

    $nombreEmpresa = $formato['nombrecomercial'];
    if ($nombreEmpresa === '') {
        $nombreEmpresa = !empty($empresa['nombrecomercial']) ? $empresa['nombrecomercial'] : $empresa['razonsocial'];
    }

    if ($formato['impresionlogo'] === '' || $formato['impresionlogo'] === null) {
        $formato['impresionlogo'] = 1;
    }
    ...
    if ($logo === '') {
        ...
        $logoPath = FCPATH . 'public/img/empresa/' . $logo;
    }
    ...
    $nombre_archivo = utf8_decode(
        $empresa['documento'] . '-' .
        $venta['oficial'] . '-' .
        $venta['seriecomprobante'] . '-' .
        $venta['nrocomprobante'] . '.pdf'
    );
    ...
    public function a4despacho($codkardex)
    {
        ...
        $venta = $this->db
            ->query(
                "SELECT kd.codkardex, kd.fechakardex, kd.movimiento, (CASE k.codmovimientotipo WHEN 20 THEN 'NOTA DE SALIDA' ELSE 'NOTA DE ENTRADA' END) as comprobante, k.codcomprobantetipo, k.codpersona,
  ...
  inner join public.personas as p on(k.codpersona=p.codpersona) where kd.codkardexalmacen=" . $codkardex,
            )
            ->result_array();
        ...
        $logo = $this->phuyu_pdf_image_src(FCPATH . 'public/img/' . $logo);
 ...
 $html .= '</tr>';
        ...
        $pdf->SetCreator(PDF_CREATOR);
        $pdf->SetAuthor('WEB phuyu');
        $pdf->SetTitle('Phuyu System | IMPRIMIR DESPACHO');
        $pdf->SetSubject('WEB phuyu');
    ...
    //     foreach ($cuentascorrientes as $key => $value) {
    //         $html .=
    //             '<p style="text-align:left !important;font-size:8px"><b>' .
    //             $value['banco'] .
    //             '</b> <br>CC: &nbsp;' .
    //             $value['nroctacte'] .
    //             '<br>
    ...
    public function a5($codkardex)
{
    ...
    $nombre = $formato['nombrecomercial'];
    if ($nombre === '') {
     ...
     public function ticket($codkardex)
    {
        ...
        SELECT
            ...
            if (empty($venta)) {
                show_error('No se encontró la venta.', 404);
                return;
            }
        ...
        $logoArchivo = !empty($formato['logo'])
            ? FCPATH . 'public/img/empresa/' . $formato['logo']
        ...
        $tot_total = (string)number_format((float)$venta['importe'], 2, '.', '');
    ...
    public function ticket2($codkardex)
    {
        if (isset($_SESSION['phuyu_codusuario'])) {
            $empresa = $this->db->query('select documento,razonsocial,nombrecomercial from public.personas where codpersona=1')->result_array();
 ...
 $archivo_error = APPPATH."/logs/qrcode.png-errors.txt";
    ...
    function a4_nota($codkardex)
    {
        if (isset($_SESSION['phuyu_codusuario'])) {
            ...
            $columnas = ['FECHA', 'T.DOC', 'N°DOC', 'DOC.IDEN', 'RAZON SOCIAL', 'VALOR VENTA', 'IGV', 'TOTAL'];
            $w = [15, 15, 20, 20, 20, 23, 10, 15];
    ...
    //     if ($codkardex <= 0) {
    //         show_error('Código de pedido inválido.');
    //     }

    //     $data = $this->obtenerDataPedido($codkardex);

    //     if (empty($data['venta'])) {
    //         show_404();
    //     }

    //     // Esto renderiza el HTML como documento en navegador
    //     $this->load->view('reportes/a4pedido', $data);
    // }
    ...
    public function a4pedido($codkardex)
    {
        ...
        $codkardex = (int) $codkardex;
        if ($codkardex <= 0) {
            show_error('Código de pedido inválido.', 400);
            return;
        }
        ...
        if (empty($data['venta'])) {
            show_error('No se encontró el pedido solicitado.', 404);
            return;
        }
        ...
        $numero = $venta['nrocomprobante'] ?? '';
        $igv = number_format((float) ($venta['igv'] ?? 0), 2, '.', '');
        $total = number_format((float) ($venta['importe'] ?? 0), 2, '.', '');
        $fecha = $venta['fechapedido'] ?? date('Y-m-d');
        ...
        $html = $this->load->view('reportes/a4pedido', $data, true);
        ...
        $nombre_archivo = 'pedido_' . $data['venta']['seriecomprobante'] . '-' . $data['venta']['nrocomprobante'] . '.pdf';

        $dompdf->stream($nombre_archivo, ['Attachment' => false]);
    }

    private function obtenerDataPedido($codkardex)
    {
    ...
    ",
        ...
        $totales = $this->db
            ->query(
                "
        SELECT
            (SELECT COALESCE(SUM(subtotal),0) FROM kardex.pedidosdetalle WHERE codpedido = {$codkardex} AND codafectacionigv = '10') AS gravado,
            (SELECT COALESCE(SUM(subtotal),0) FROM kardex.pedidosdetalle WHERE codpedido = {$codkardex} AND codafectacionigv = '20') AS exonerado,
            (SELECT COALESCE(SUM(subtotal),0) FROM kardex.pedidosdetalle WHERE codpedido = {$codkardex} AND codafectacionigv = '30') AS inafecto,
            (SELECT COALESCE(SUM(subtotal),0) FROM kardex.pedidosdetalle WHERE codpedido = {$codkardex} AND codafectacionigv = '21') AS gratuito
    ",
            )
            ->row_array();
        ...
        SELECT
            ...
            kd.subtotal,
            kd.descripcion
        FROM kardex.pedidosdetalle AS kd
        INNER JOIN almacen.productos AS p ON p.codproducto = kd.codproducto
        INNER JOIN almacen.unidades AS u ON u.codunidad = kd.codunidad
        WHERE kd.codpedido = {$codkardex}
        ORDER BY kd.item
    ",
            )
            ->result_array();
        ...
        SELECT *
        FROM caja.comprobantes
        WHERE codcomprobantetipo = {$venta['codcomprobantetipo']}
          AND seriecomprobante = " .
                    $this->db->escape($venta['seriecomprobante']) .
                    "
    ...
    ",
        ...
        $nombre_empresa = !empty($empresa['nombrecomercial']) ? $empresa['nombrecomercial'] : $empresa['razonsocial'];

        $logo = 'empresa/default.png';
        if (!empty($formato['logo'])) {
            $logo = 'empresa/' . $formato['logo'];
        }
        ...
        $parte_entera = floor($importe);
        $parte_decimal = (int) round(($importe - $parte_entera) * 100);
        ...
        $total_texto = strtoupper($number->convertirNumeroEnLetras($parte_entera) . ' Y ' . str_pad($parte_decimal, 2, '0', STR_PAD_LEFT) . '/100 SOLES');
        ...
        return [
            ...
            'logo_url' => base_url('public/img/' . $logo),
            'total_texto' => $total_texto,
    ...
    public function a5pedido($codkardex)
    {
        ...
        $venta = $this->db
            ->query(
                "select k.codpedido,k.fechapedido,ct.descripcion as comprobante, k.codcomprobantetipo, v.razonsocial as vendedor,pu.razonsocial as usuario ,
     ...
     inner join seguridad.usuarios as u on(k.codusuario=u.codusuario)
     inner join public.personas as pu on(u.codempleado=pu.codpersona)
     inner join caja.comprobantetipos as ct on(k.codcomprobantetipo=ct.codcomprobantetipo) where k.codpedido=" . $codkardex,
            )
            ->result_array();
