...
class Facturacion extends Sunat
{
	...
	// Función modificada: resumenes_enviar
	// Notas: usa rutas absolutas (FCPATH si está definido), logging en sunat/logs, y fuerza que WSSE Username sea RUC+USUARIOSOL
		...
		if (defined('FCPATH')) {	$basePath = rtrim(FCPATH, '/') . '/';} 
			...
			// Ajusta según estructura: este archivo está en controllers/facturacion
		...
		$resumen = $this->db->query("select * from sunat.resumenes where codresumentipo=? and periodo=? and nrocorrelativo=?", [$codresumentipo, $periodo, $nrocorrelativo])->result_array();
		if (empty($resumen)) {
			echo json_encode(["estado" => 0, "mensaje" => "resumen_no_encontrado"]);
			return;
		}

		// Cambia `codempresa` segun corresponda; usé 1 como en tu ejemplo
		$empresa = $this->db->query("select * from public.webservice where codempresa=?", [$_SESSION["phuyu_codempresa"]])->result_array();
		if (empty($empresa)) {
			echo json_encode(["estado" => 0, "mensaje" => "config_webservice_no_encontrada"]);
			return;
		}
		$empresa = $empresa[0];
		...
		try {
			...
			if (!empty($resumen[0]["ticket"])) {
				...
				$estado_final = isset($resp["estado"]) ? $resp["estado"] : 0;

				$log("Consulta TICKET resultado estado={$estado_final} mensaje=" . $mensaje);
			} else {
				// No hay ticket: generar XML
				if ($codresumentipo == 1) {
					$generar = $this->Facturacion_model->phuyu_rf_crearXML($periodo, $nrocorrelativo);
				} else {
					$generar = $this->Facturacion_model->phuyu_rb_crearXML($periodo, $nrocorrelativo, $codresumentipo);
				}

				if (!is_array($generar) || !isset($generar['estado']) || $generar['estado'] == 0) {
					$log("Fallo generar XML: " . json_encode($generar));
					echo json_encode(["estado" => 0, "mensaje" => "NO SE PUEDE GENERAR EL DOCUMENTO XML"]);
					return;
				}
				...
				if ($firma !== 1) {
					...
					echo json_encode(["estado" => 0, "mensaje" => "NO SE PUEDE FIRMAR EL DOCUMENTO XML"]);
					return;
				}
	...
	function resumenes_siguiente_correlativo($codresumentipo, $periodo, $nrocorrelativo)
	{
		if ($this->input->is_ajax_request()) {
			...
			$valores = [
				(int)$codresumentipo,
				$periodo,
				(int)$nrocorrelativo_nuevo,
				(int)$_SESSION["phuyu_codempresa"],
				(int)$_SESSION["phuyu_codsucursal"],
				(int)$_SESSION["phuyu_codusuario"],
				$xml,
				$resumen[0]["fecharesumen"]
			];
			$estado = $this->phuyu_model->phuyu_guardar("sunat.resumenes", $campos, $valores);
