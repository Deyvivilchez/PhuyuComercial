...
class Facturacion_model extends CI_Model
{
    ...
    function phuyu_crearXML($codcomprobante, $codkardex)
    {
        ...
        $cbc = $xml->createElement("cbc:ID", $info[0]["seriecomprobante"] . "-" . $info[0]["nrocomprobante"]);
        $cbc = $Invoice->appendChild($cbc);
        $cbc = $xml->createElement("cbc:IssueDate", $info[0]["fechacomprobante"]);
        $cbc = $Invoice->appendChild($cbc);

        // 5. CODIGO TIPO DOCUMENTO CATALOGO 01 //

        if ($codcomprobante <> "07" && $codcomprobante <> "08") {
            $cbc = $xml->createElement("cbc:InvoiceTypeCode", $codcomprobante);
            $cbc = $Invoice->appendChild($cbc);
        ...
        $cbc = $xml->createElement("cbc:Note", $texto_importe);
        $cbc = $Invoice->appendChild($cbc);
        ...
        $cbc = $xml->createElement("cbc:LineCountNumeric", count($detalle));
        $cbc = $Invoice->appendChild($cbc);
        ...
        if ($codcomprobante == "08" || $codcomprobante == "07") {
            ...
            $cac_discre = $xml->createElement('cac:DiscrepancyResponse');
            $cac_discre = $Invoice->appendChild($cac_discre);
            //verificar la letra S  
            $cbc = $xml->createElement('cbc:ReferenceID', $info[0]["seriecomprobante_ref"] . "-" . trim($info[0]["nrocomprobante_ref"]));
            $cbc = $cac_discre->appendChild($cbc);
            $cbc = $xml->createElement('cbc:ResponseCode', $consulta_motivo[0]["oficial"]);
            $cbc = $cac_discre->appendChild($cbc);
            //el espacio vacio es para cuando no aya datos
            $cbc = $xml->createElement('cbc:Description', $info[0]["descripcion"] . ' ');
            $cbc = $cac_discre->appendChild($cbc);

            //
            $cac_billin = $xml->createElement('cac:BillingReference');
            $cac_billin = $Invoice->appendChild($cac_billin);
            $cac = $xml->createElement('cac:InvoiceDocumentReference');
            $cac = $cac_billin->appendChild($cac);
            $cbc = $xml->createElement('cbc:ID', $info[0]["seriecomprobante_ref"] . "-" . trim($info[0]["nrocomprobante_ref"]));
            $cbc = $cac->appendChild($cbc);
            $cbc = $xml->createElement('cbc:DocumentTypeCode', $codcomprobantetipo_ofi[0]["oficial"]);
            $cbc = $cac->appendChild($cbc);
        }
        ...
        $cbc = $xml->createElement("cbc:ID", "SignSUNAT");
        ...
        $cbc = $xml->createElement("cbc:ID", $empresa[0]["documento"]);
        $cbc = $cac_partyidentification->appendChild($cbc);
        ...
        $cbc = $xml->createElement("cbc:URI", "#SignSUNAT");
        $cbc = $cac_externalreference->appendChild($cbc);
        ...
        $cac_legal = $xml->createElement("cac:PartyLegalEntity");
        $cac_legal = $cac_party->appendChild($cac_legal);
        ...
        $cbc = $xml->createElement("cbc:ID", $empresa[0]["ubigeo"]);
        $cbc = $cac_adres->appendChild($cbc);
        ...
        $cbc = $xml->createElement("cbc:CitySubdivisionName", "-");
        $cbc = $cac_adres->appendChild($cbc);
        $cbc = $xml->createElement("cbc:CityName", $empresa[0]["departamento"]);
        $cbc = $cac_adres->appendChild($cbc);
        $cbc = $xml->createElement("cbc:CountrySubentity", $empresa[0]["provincia"]);
        $cbc = $cac_adres->appendChild($cbc);
        $cbc = $xml->createElement("cbc:District", $empresa[0]["distrito"]);
        $cbc = $cac_adres->appendChild($cbc);

        $cac_dir = $xml->createElement("cac:AddressLine");
        $cac_dir = $cac_adres->appendChild($cac_dir);
        $cbc = $xml->createElement("cbc:Line", $empresa[0]["direccion"]);
        $cbc = $cac_dir->appendChild($cbc);

        $cac_ciu = $xml->createElement("cac:Country");
        $cac_ciu = $cac_adres->appendChild($cac_ciu);
        $cbc = $xml->createElement("cbc:IdentificationCode", "PE");
        $cbc = $cac_ciu->appendChild($cbc);
        ...
        $cac_partyidentification = $xml->createElement("cac:PartyIdentification");
        $cac_partyidentification = $cac_party->appendChild($cac_partyidentification);
        $cbc = $xml->createElement("cbc:ID", $info[0]["documento"]);
        $cbc = $cac_partyidentification->appendChild($cbc);
        ...
        $cac_dir = $xml->createElement("cac:AddressLine");
        $cac_dir = $cac_adres->appendChild($cac_dir);
        $cbc = $xml->createElement("cbc:Line", $info[0]["direccion"]);
        $cbc = $cac_dir->appendChild($cbc);
        ...
        if ($codcomprobante <> "07" && $codcomprobante <> "08") {
            ...
            $cac_condicion = $xml->createElement("cac:PaymentTerms");
            $cac_condicion = $Invoice->appendChild($cac_condicion);
            $cbc = $xml->createElement("cbc:ID", "FormaPago");
            $cbc = $cac_condicion->appendChild($cbc);
            $cbc = $xml->createElement("cbc:PaymentMeansID", $textcondicion);
            $cbc = $cac_condicion->appendChild($cbc);
            $cbc = $xml->createElement("cbc:Amount", number_format($info[0]["importe"], 2, ".", ""));
            $cbc = $cac_condicion->appendChild($cbc);
            ...
            if ($info[0]["condicionpago"] == 2) {
                ...
                foreach ($cuotas as $key => $value) {
                    ...
                    $item = 'Cuota' . $c;
                    $cac_cuotas = $xml->createElement("cac:PaymentTerms");
                    $cac_cuotas = $Invoice->appendChild($cac_cuotas);
                    $cbc = $xml->createElement("cbc:ID", "FormaPago");
                    $cbc = $cac_cuotas->appendChild($cbc);
                    $cbc = $xml->createElement("cbc:PaymentMeansID", $item);
                    $cbc = $cac_cuotas->appendChild($cbc);
                    $cbc = $xml->createElement("cbc:Amount", number_format($value["importe"], 2, ".", ""));
                    $cbc = $cac_cuotas->appendChild($cbc);
                    $cbc->setAttribute("currencyID", "PEN");
                    $cbc = $xml->createElement("cbc:PaymentDueDate", $value["fechavence"]);
                    $cbc = $cac_cuotas->appendChild($cbc);
                    $c++;
                }
            }
        ...
        $cac_total = $xml->createElement("cac:TaxTotal");
        $cac_total = $Invoice->appendChild($cac_total);
        $cbc = $xml->createElement("cbc:TaxAmount", number_format(($info[0]["igv"] + $info[0]["icbper"]), 2, ".", ""));
        $cbc = $cac_total->appendChild($cbc);
        $cbc->setAttribute("currencyID", "PEN");

        // 10.1: SUBTOTAL OPERACIONES GRAVADAS //

        if ($totales[0]["gravado"] > 0) {
            $cac_subigv = $xml->createElement("cac:TaxSubtotal");
            $cac_subigv = $cac_total->appendChild($cac_subigv);
            $cbc = $xml->createElement("cbc:TaxableAmount", number_format($totales[0]["gravado"], 2, ".", ""));
            $cbc = $cac_subigv->appendChild($cbc);
            $cbc->setAttribute("currencyID", "PEN");
            $cbc = $xml->createElement('cbc:TaxAmount', number_format($info[0]["igv"], 2, ".", ""));
            $cbc = $cac_subigv->appendChild($cbc);
            $cbc->setAttribute('currencyID', 'PEN');

            $cac_cat = $xml->createElement("cac:TaxCategory");
            $cac_cat = $cac_subigv->appendChild($cac_cat);
            $cac_esq = $xml->createElement("cac:TaxScheme");
            $cac_esq = $cac_cat->appendChild($cac_esq);
            $cbc = $xml->createElement("cbc:ID", "1000");
            $cbc = $cac_esq->appendChild($cbc);
            $cbc->setAttribute("schemeAgencyName", "PE:SUNAT");
        ...
        if ($totales[0]["exonerado"] > 0) {
            $cac_subtal = $xml->createElement("cac:TaxSubtotal");
            $cac_subtal = $cac_total->appendChild($cac_subtal);
            $cbc = $xml->createElement("cbc:TaxableAmount", number_format($totales[0]["exonerado"], 2, ".", ""));
            $cbc = $cac_subtal->appendChild($cbc);
            $cbc->setAttribute("currencyID", "PEN");
            ...
            $cac_cat = $xml->createElement("cac:TaxCategory");
            $cac_cat = $cac_subtal->appendChild($cac_cat);
            $cac_esq = $xml->createElement("cac:TaxScheme");
            $cac_esq = $cac_cat->appendChild($cac_esq);
        ...
        if ($totales[0]["gratuito"] > 0) {
            $cac_subtal = $xml->createElement("cac:TaxSubtotal");
            $cac_subtal = $cac_total->appendChild($cac_subtal);
            $cbc = $xml->createElement("cbc:TaxableAmount", number_format($totales[0]["gratuito"], 2, ".", ""));
            $cbc = $cac_subtal->appendChild($cbc);
            $cbc->setAttribute("currencyID", "PEN");
            $cbc = $xml->createElement("cbc:TaxAmount", "0.00");
            $cbc = $cac_subtal->appendChild($cbc);
            ...
            $cac_esq = $xml->createElement("cac:TaxScheme");
            $cac_esq = $cac_cat->appendChild($cac_esq);
            $cbc = $xml->createElement("cbc:ID", "9996");
            $cbc = $cac_esq->appendChild($cbc);
            $cbc->setAttribute("schemeAgencyName", "PE:SUNAT");
            $cbc->setAttribute("schemeID", "UN/ECE 5153");
            $cbc->setAttribute("schemeName", "Codigo de tributos");
            $cbc = $xml->createElement("cbc:Name", "GRA");
            $cbc = $cac_esq->appendChild($cbc);
            $cbc = $xml->createElement("cbc:TaxTypeCode", "FRE");
            $cbc = $cac_esq->appendChild($cbc);
        }
        ...
        if ($info[0]["icbper"] > 0) {
            $cac_icbper = $xml->createElement("cac:TaxSubtotal");
            $cac_icbper = $cac_total->appendChild($cac_icbper);
            $cbc = $xml->createElement("cbc:TaxAmount", number_format($info[0]["icbper"], 2, ".", ""));
            $cbc = $cac_icbper->appendChild($cbc);
            $cbc->setAttribute("currencyID", "PEN");

            $cac_cat = $xml->createElement("cac:TaxCategory");
            $cac_cat = $cac_icbper->appendChild($cac_cat);
            $cac_esq = $xml->createElement("cac:TaxScheme");
            $cac_esq = $cac_cat->appendChild($cac_esq);
            $cbc = $xml->createElement("cbc:ID", "7152");
            $cbc = $cac_esq->appendChild($cbc);
            $cbc = $xml->createElement("cbc:Name", "ICBPER");
            $cbc = $cac_esq->appendChild($cbc);
        ...
        if ($codcomprobante <> '07' && $codcomprobante <> '08') {
            $cac_total = $xml->createElement("cac:LegalMonetaryTotal");
            $cac_total = $Invoice->appendChild($cac_total);
            $cbc = $xml->createElement("cbc:LineExtensionAmount", number_format($info[0]["valorventa"], 2, ".", ""));
            $cbc = $cac_total->appendChild($cbc);
            $cbc->setAttribute("currencyID", "PEN");

            $cbc = $xml->createElement("cbc:TaxInclusiveAmount", number_format($info[0]["importe"], 2, '.', ''));
            $cbc = $cac_total->appendChild($cbc);
            $cbc->setAttribute("currencyID", "PEN");

            $cbc = $xml->createElement("cbc:AllowanceTotalAmount", number_format($info[0]["descglobal"], 2, ".", ""));
            $cbc = $cac_total->appendChild($cbc);
            $cbc->setAttribute("currencyID", "PEN");
            $cbc = $xml->createElement("cbc:ChargeTotalAmount", "0.00");
            $cbc = $cac_total->appendChild($cbc);
            $cbc->setAttribute("currencyID", "PEN");
            $cbc = $xml->createElement("cbc:PrepaidAmount", "0.00");
            $cbc = $cac_total->appendChild($cbc);
            $cbc->setAttribute("currencyID", "PEN");
            $cbc = $xml->createElement("cbc:PayableAmount", number_format($info[0]["importe"], 2, ".", ""));
            $cbc = $cac_total->appendChild($cbc);
            $cbc->setAttribute("currencyID", "PEN");
        } else {
            $cac_total = $xml->createElement("cac:LegalMonetaryTotal");
        ...
        foreach ($detalle as $key => $value) {
            if ($codcomprobante <> "07" && $codcomprobante <> "08") {
                ...
                $line = $xml->createElement("cac:DebitNoteLine");
                $line = $Invoice->appendChild($line);
            }

            // 12.1: NRO DE ITEM / CANTIDAD Y UNIDAD DE MEDIDA //

            $cbc = $xml->createElement("cbc:ID", $value["item"]);
            ...
            $cbc = $xml->createElement("cbc:LineExtensionAmount", number_format($value["valorventa"], 2, ".", ""));
            $cbc = $line->appendChild($cbc);
            $cbc->setAttribute("currencyID", "PEN");

            $precios = $xml->createElement("cac:PricingReference");
            $precios = $line->appendChild($precios);
            $precios_al = $xml->createElement("cac:AlternativeConditionPrice");
            $precios_al = $precios->appendChild($precios_al);

            $precioref = (float)($value["subtotal"] / $value["cantidad"]);
            $cbc = $xml->createElement("cbc:PriceAmount", number_format($precioref, 2, ".", ""));
            $cbc = $precios_al->appendChild($cbc);
            $cbc->setAttribute("currencyID", "PEN");

            if ($value["codafectacionigv"] == 21) {
                $cbc = $xml->createElement("cbc:PriceTypeCode", "02");
                $cbc = $precios_al->appendChild($cbc);
                $cbc->setAttribute("listAgencyName", "PE:SUNAT");
            ...
            //             $cbc->setAttribute("currencyID","PEN");
            // }
            if ( isset($value["porcdescuento"], 
                $value["descuento"]) &&
                (float)$value["porcdescuento"] > 0 &&
                (float)$value["descuento"] > 0
                 ) {
                $baseDescuento = (float)$value["subtotal"];

                if ($baseDescuento > 0) {
                    $descuentos = $xml->createElement("cac:AllowanceCharge");
                    $descuentos = $line->appendChild($descuentos);

                    $cbc = $xml->createElement("cbc:ChargeIndicator", "false");
                    $cbc = $descuentos->appendChild($cbc);

                    $cbc = $xml->createElement("cbc:AllowanceChargeReasonCode", "00");
                    $cbc = $descuentos->appendChild($cbc);

                    $cbc = $xml->createElement("cbc:Amount", number_format((float)$value["descuento"], 2, ".", ""));
                    $cbc = $descuentos->appendChild($cbc);
                    $cbc->setAttribute("currencyID", "PEN");

                    $cbc = $xml->createElement("cbc:BaseAmount", number_format($baseDescuento, 2, ".", ""));
                    $cbc = $descuentos->appendChild($cbc);
                    $cbc->setAttribute("currencyID", "PEN");
                }
            }
            ...
            $cac_tot = $xml->createElement("cac:TaxTotal");
            $cac_tot = $line->appendChild($cac_tot);
            $cbc = $xml->createElement("cbc:TaxAmount", number_format(($value["igv"] + $value["icbper"]), 2, ".", ""));
            $cbc = $cac_tot->appendChild($cbc);
            $cbc->setAttribute("currencyID", "PEN");

            // 12.4.1 ITEM GRAVADO //

            if ($value["codafectacionigv"] == 10) {
                $cac_ig = $xml->createElement("cac:TaxSubtotal");
                $cac_ig = $cac_tot->appendChild($cac_ig);
                $cbc = $xml->createElement("cbc:TaxableAmount", number_format($value["valorventa"], 2, ".", ""));
                $cbc = $cac_ig->appendChild($cbc);
                $cbc->setAttribute("currencyID", "PEN");
                $cbc = $xml->createElement("cbc:TaxAmount", number_format($value["igv"], 2, ".", ""));
                $cbc = $cac_ig->appendChild($cbc);
                $cbc->setAttribute("currencyID", "PEN");

                $cac_ct = $xml->createElement("cac:TaxCategory");
                $cac_ct = $cac_ig->appendChild($cac_ct);

                $cbc = $xml->createElement("cbc:Percent", "18.00");
                $cbc = $cac_ct->appendChild($cbc);
                ...
                $cac_sq = $xml->createElement("cac:TaxScheme");
                $cac_sq = $cac_ct->appendChild($cac_sq);
                $cbc = $xml->createElement("cbc:ID", "1000");
                $cbc = $cac_sq->appendChild($cbc);
                $cbc->setAttribute("schemeAgencyName", "PE:SUNAT");
                $cbc->setAttribute("schemeID", "UN/ECE 5153");
                $cbc->setAttribute("schemeName", "Codigo de tributos");
            ...
            if ($value["codafectacionigv"] == 20) {
                $cac_ig = $xml->createElement("cac:TaxSubtotal");
                $cac_ig = $cac_tot->appendChild($cac_ig);
                $cbc = $xml->createElement("cbc:TaxableAmount", number_format($value["subtotal"], 2, ".", ""));
                $cbc = $cac_ig->appendChild($cbc);
                $cbc->setAttribute("currencyID", "PEN");
                $cbc = $xml->createElement("cbc:TaxAmount", "0.00");
                $cbc = $cac_ig->appendChild($cbc);
                ...
                $cbc = $xml->createElement("cbc:Percent", "0.00");
                $cbc = $cac_ct->appendChild($cbc);
                $cbc = $xml->createElement("cbc:TaxExemptionReasonCode", "20");
                $cbc = $cac_ct->appendChild($cbc);
                $cbc->setAttribute("listAgencyName", "PE:SUNAT");
                $cbc->setAttribute("listName", "Afectacion del IGV");
                $cbc->setAttribute("listURI", "urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo07");
            ...
            if ($value["codafectacionigv"] == 21) {
                $cac_ig = $xml->createElement("cac:TaxSubtotal");
                $cac_ig = $cac_tot->appendChild($cac_ig);
                $cbc = $xml->createElement("cbc:TaxableAmount", number_format($value["preciorefunitario"] * $value["cantidad"], 2, ".", ""));
                $cbc = $cac_ig->appendChild($cbc);
                $cbc->setAttribute("currencyID", "PEN");
                $cbc = $xml->createElement("cbc:TaxAmount", "0.00");
                $cbc = $cac_ig->appendChild($cbc);
                ...
                $cbc = $xml->createElement("cbc:Percent", "0.00");
                $cbc = $cac_ct->appendChild($cbc);
                $cbc = $xml->createElement("cbc:TaxExemptionReasonCode", "21");
                $cbc = $cac_ct->appendChild($cbc);
                $cbc->setAttribute("listAgencyName", "PE:SUNAT");
                $cbc->setAttribute("listName", "Afectacion del IGV");
                $cbc->setAttribute("listURI", "urn:pe:gob:sunat:cpe:see:gem:catalogos:catalogo07");
            ...
            if ($value["icbper"] > 0) {
                $cac_ig = $xml->createElement("cac:TaxSubtotal");
                $cac_ig = $cac_tot->appendChild($cac_ig);
                $cbc = $xml->createElement("cbc:TaxAmount", number_format($value["subtotal"], 2, ".", ""));
                $cbc = $cac_ig->appendChild($cbc);
                $cbc->setAttribute("currencyID", "PEN");
            ...
            $cbc = $xml->createElement("cbc:ID", $value["codproducto"]);
            $cbc = $sellers->appendChild($cbc);

            $price = $xml->createElement("cac:Price");
            $price = $line->appendChild($price);
            $cbc = $xml->createElement("cbc:PriceAmount", number_format($value["preciosinigv"], 4, ".", ""));
        ...
        $xml->save($carpeta_phuyu . "/" . $info[0]["nombre_xml"] . ".xml");
        chmod($carpeta_phuyu . "/" . $info[0]["nombre_xml"] . ".xml", 0777);

        if (file_exists($carpeta_phuyu . "/" . $info[0]["nombre_xml"] . ".xml")) {
            $data["estado"] = 1;
            $data["carpeta_phuyu"] = $carpeta_phuyu;
            $data["archivo_phuyu"] = $info[0]["nombre_xml"];
        } else {
            $data["estado"] = 0;
        }
        return $data;
    ...
    function phuyu_crearXMLGUIAS($codcomprobante, $codguiar)
    {
        ...
        $cbc = $xml->createElement("cbc:ID", $info[0]["seriecomprobante"] . "-" . $info[0]["nrocomprobante"]);
        $cbc = $Invoice->appendChild($cbc);
        $cbc = $xml->createElement("cbc:IssueDate", $info[0]["fechaguia"]);
        $cbc = $Invoice->appendChild($cbc);

        $cbc = $xml->createElement("cbc:IssueTime", '00:00:00');
        $cbc = $Invoice->appendChild($cbc);


        // 5. CODIGO TIPO DOCUMENTO CATALOGO 01 //

        $cbc = $xml->createElement("cbc:DespatchAdviceTypeCode", $codcomprobante);
        $cbc = $Invoice->appendChild($cbc);

        // 6. OBSERVACION

        $cbc = $xml->createElement("cbc:Note", trim($info[0]["observaciones"]));
        $cbc = $Invoice->appendChild($cbc);
        ...
        if ($info[0]["codmotivotraslado"] == 2) {
            ...
            $cac_party = $xml->createElement("cac:Party");
            $cac_party = $cac_proveedor->appendChild($cac_party);
        ...
        $cac_sepelier = $xml->createElement("cac:Shipment");
        $cac_sepelier = $Invoice->appendChild($cac_sepelier);

        $cbc = $xml->createElement("cbc:ID", "1");
        $cbc = $cac_sepelier->appendChild($cbc);

        $cbc = $xml->createElement("cbc:HandlingCode", $info[0]["oficialmotivo"]);
        $cbc = $cac_sepelier->appendChild($cbc);
        $cbc = $xml->createElement("cbc:Information", $info[0]["motivo"]);
        $cbc = $cac_sepelier->appendChild($cbc);

        $cbc = $xml->createElement("cbc:GrossWeightMeasure", number_format($info[0]["peso"], 2, ".", ""));
        $cbc = $cac_sepelier->appendChild($cbc);
        $cbc->setAttribute("unitCode", "KGM");

        $cbc = $xml->createElement("cbc:TotalTransportHandlingUnitQuantity", $info[0]["nropaquetes"]);
        $cbc = $cac_sepelier->appendChild($cbc);

        $cac_shipment = $xml->createElement("cac:ShipmentStage");
        $cac_shipment = $cac_sepelier->appendChild($cac_shipment);

        $cbc = $xml->createElement("cbc:TransportModeCode", '0' . $info[0]["codmodalidadtraslado"]);
        $cbc = $cac_shipment->appendChild($cbc);

        $cac = $xml->createElement("cac:TransitPeriod");
        $cac = $cac_shipment->appendChild($cac);

        $cbc = $xml->createElement("cbc:StartDate", $info[0]["fechatraslado"]);
        $cbc = $cac->appendChild($cbc);

        if ($info[0]["codmodalidadtraslado"] == 1) {

            //EMPRESA TRANSPORTISTA
            $cac_transporte = $xml->createElement("cac:CarrierParty");
            $cac_transporte = $cac_shipment->appendChild($cac_transporte);
            $cac_partyidentification = $xml->createElement("cac:PartyIdentification");
            $cac_partyidentification = $cac_transporte->appendChild($cac_partyidentification);
            $cbc = $xml->createElement("cbc:ID", $info[0]["documentotransportista"]);
            $cbc = $cac_partyidentification->appendChild($cbc);
            $cbc->setAttribute("schemeID", "6");

            $cac_partyName = $xml->createElement("cac:PartyName");
            $cac_partyName = $cac_transporte->appendChild($cac_partyName);
            $cbc = $xml->createElement("cbc:Name", htmlspecialchars($info[0]["razonsocialtransportista"]));
            $cbc = $cac_partyName->appendChild($cbc);
        } else {

            //VEHICULO 
            $cac_vehiculo = $xml->createElement("cac:TransportMeans");
            $cac_vehiculo = $cac_shipment->appendChild($cac_vehiculo);
            $cac_transport = $xml->createElement("cac:RoadTransport");
            $cac_transport = $cac_vehiculo->appendChild($cac_transport);

            $cbc = $xml->createElement("cbc:LicensePlateID", $info[0]["licenciaconductor"]);
            $cbc = $cac_transport->appendChild($cbc);


            //CONDUCTOR
            $cac_conductor = $xml->createElement("cac:DriverPerson");
            $cac_conductor = $cac_shipment->appendChild($cac_conductor);
            $cbc = $xml->createElement("cbc:ID", $info[0]["documentoconductor"]);
            $cbc = $cac_conductor->appendChild($cbc);
        ...
        $cac_delivery = $cac_sepelier->appendChild($cac_delivery);
        $cac_deliveryadress = $xml->createElement("cac:DeliveryAddress");
        $cac_delivery->appendChild($cac_deliveryadress);
        $cbc = $xml->createElement("cbc:ID", $ubigeollegada[0]["ubidepartamento"] . $ubigeollegada[0]["ubiprovincia"] . $ubigeollegada[0]["ubidistrito"]);
        $cbc = $cac_deliveryadress->appendChild($cbc);
        $cbc = $xml->createElement("cbc:StreetName", htmlspecialchars((string)$info[0]["direccionllegada"]));
        $cbc = $cac_deliveryadress->appendChild($cbc);

        $ubigeopartida = $this->db->query("select *from ubigeo WHERE codubigeo=" . $info[0]["codubigeopartida"])->result_array();

        $cac_origin = $xml->createElement("cac:OriginAddress");
        $cac_origin = $cac_sepelier->appendChild($cac_origin);
        $cbc = $xml->createElement("cbc:ID", $ubigeopartida[0]["ubidepartamento"] . $ubigeopartida[0]["ubiprovincia"] . $ubigeopartida[0]["ubidistrito"]);
        $cbc = $cac_origin->appendChild($cbc);
        $cbc = $xml->createElement("cbc:StreetName", htmlspecialchars($info[0]["direccionpartida"]));
        $cbc = $cac_origin->appendChild($cbc);
        ...
        foreach ($detalle as $key => $value) {
            ...
            $cbc = $xml->createElement("cbc:LineID", $value["item"]);
            $cbc = $cac_orden->appendChild($cbc);

            $cac_item = $xml->createElement("cac:Item");
            $cac_item = $line->appendChild($cac_item);
            $cbc = $xml->createElement("cbc:Name", htmlspecialchars($value["producto"]));
            $cbc = $cac_item->appendChild($cbc);

            $cac_seller = $xml->createElement("cac:SellersItemIdentification");
            $cac_seller = $cac_item->appendChild($cac_seller);
            $cbc = $xml->createElement("cbc:ID", $value["codproducto"]);
            $cbc = $cac_seller->appendChild($cbc);
        }
    ...
    function phuyu_rf_crearXML($periodo, $nrocorrelativo)
    {
        ...
        $cbc = $Invoice->appendChild($cbc);
        $cbc = $xml->createElement("cbc:ReferenceDate", $resumen[0]["fecharesumen"]);
        $cbc = $Invoice->appendChild($cbc);
        $cbc = $xml->createElement("cbc:IssueDate", $resumen[0]["fecharesumen"]);
        $cbc = $Invoice->appendChild($cbc);
        ...
        $cbc = $xml->createElement("cbc:CustomerAssignedAccountID", $empresa[0]["documento"]);
        ...
        foreach ($detalle as $key => $value) {
            ...
            $cbc = $xml->createElement("cbc:LineID", $item);
            $cbc = $VoidedDocumentsLine->appendChild($cbc);
            $cbc = $xml->createElement("cbc:DocumentTypeCode", "01");
            $cbc = $VoidedDocumentsLine->appendChild($cbc);
            $sac = $xml->createElement("sac:DocumentSerialID", $value["seriecomprobante"]);
            $sac = $VoidedDocumentsLine->appendChild($sac);
            $sac = $xml->createElement("sac:DocumentNumberID", $value["nrocomprobante"]);
            $sac = $VoidedDocumentsLine->appendChild($sac);
            $sac = $xml->createElement("sac:VoidReasonDescription", $value["motivobaja"]);
            $sac = $VoidedDocumentsLine->appendChild($sac);
        }
        ...
        if (file_exists($carpeta_phuyu . "/" . $resumen[0]["nombre_xml"] . ".xml")) {
            $data["estado"] = 1;
            $data["carpeta_phuyu"] = $carpeta_phuyu;
            $data["archivo_phuyu"] = $resumen[0]["nombre_xml"];
        } else {
            $data["estado"] = 0;
        }
        return $data;
    ...
    function phuyu_rb_crearXML($periodo, $nrocorrelativo, $codresumentipo)
    {
        ...
        foreach ($detalle as $key => $value) {
            $item = $item + 1;
            $summary = $xml->createElement("sac:SummaryDocumentsLine");
            $summary = $Invoice->appendChild($summary);
            $cbc = $xml->createElement("cbc:LineID", $item);
            $cbc = $summary->appendChild($cbc);
            $cbc = $xml->createElement("cbc:DocumentTypeCode", "03");
            $cbc = $summary->appendChild($cbc);
            $cbc = $xml->createElement("cbc:ID", $value["seriecomprobante"] . "-" . $value["nrocomprobante"]);
            $cbc = $summary->appendChild($cbc);

            $cac_cli = $xml->createElement("cac:AccountingCustomerParty");
            $cac_cli = $summary->appendChild($cac_cli);
            $cbc = $xml->createElement("cbc:CustomerAssignedAccountID", $value["documento"]);
            $cbc = $cac_cli->appendChild($cbc);
            $cbc = $xml->createElement("cbc:AdditionalAccountID", $value["coddocumento"]);
            $cbc = $cac_cli->appendChild($cbc);

            $cac_estado = $xml->createElement("cac:Status");
            $cac_estado = $summary->appendChild($cac_estado);
            $cbc = $xml->createElement("cbc:ConditionCode", $estado);
            $cbc = $cac_estado->appendChild($cbc);

            $cbc = $xml->createElement("sac:TotalAmount", number_format($value["importe"], 2, ".", ""));
            $cbc = $summary->appendChild($cbc);
            $cbc->setAttribute("currencyID", "PEN");
            ...
            if ($gravado[0]["importe"] > 0) {
                $cac_pay = $xml->createElement("sac:BillingPayment");
                $cac_pay = $summary->appendChild($cac_pay);
                $cbc = $xml->createElement("cbc:PaidAmount", number_format($gravado[0]["importe"], 2, ".", ""));
                $cbc = $cac_pay->appendChild($cbc);
                $cbc->setAttribute("currencyID", "PEN");
            ...
            if ($exonerado[0]["importe"] > 0) {
                $cac_pay = $xml->createElement("sac:BillingPayment");
                $cac_pay = $summary->appendChild($cac_pay);
                $cbc = $xml->createElement("cbc:PaidAmount", number_format($exonerado[0]["importe"], 2, ".", ""));
                $cbc = $cac_pay->appendChild($cbc);
                $cbc->setAttribute("currencyID", "PEN");
            ...
            if ($inafecto[0]["importe"] > 0) {
                $cac_pay = $xml->createElement("sac:BillingPayment");
                $cac_pay = $summary->appendChild($cac_pay);
                $cbc = $xml->createElement("cbc:PaidAmount", number_format($inafecto[0]["importe"], 2, ".", ""));
                $cbc = $cac_pay->appendChild($cbc);
                $cbc->setAttribute("currencyID", "PEN");
            ...
            $cac_total = $xml->createElement("cac:TaxTotal");
            $cac_total = $summary->appendChild($cac_total);
            $cbc = $xml->createElement("cbc:TaxAmount", "0.00");
            $cbc = $cac_total->appendChild($cbc);
            $cbc->setAttribute("currencyID", "PEN");

            $cac_sub = $xml->createElement("cac:TaxSubtotal");
            $cac_sub = $cac_total->appendChild($cac_sub);
            $cbc = $xml->createElement("cbc:TaxAmount", "0.00");
            $cbc = $cac_sub->appendChild($cbc);
            $cbc->setAttribute("currencyID", "PEN");
            $cac_cat = $xml->createElement("cac:TaxCategory");
            ...
            $cac_total = $xml->createElement("cac:TaxTotal");
            $cac_total = $summary->appendChild($cac_total);
            $cbc = $xml->createElement("cbc:TaxAmount", number_format($value["icbper"], 2, ".", ""));
            $cbc = $cac_total->appendChild($cbc);
            $cbc->setAttribute("currencyID", "PEN");
