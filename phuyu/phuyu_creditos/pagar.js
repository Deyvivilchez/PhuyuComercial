var phuyu_cobranza = new Vue({
	...
	data: {
		campos:{
			"codpersona":phuyu_creditos.registro,"codlote":phuyu_creditos.codlote,"codmoneda":1,"tipocambio":0.00,"codconcepto":17,"codcomprobantetipo":2,"codtipopago":1,"importe":0,"vuelto":0,
	...
	methods: {
		...
		phuyu_cobrar: function(index,cuota){
			if ($("#"+index).is(":checked")){
				...
				this.campos.total = Number(( (this.campos.total + parseFloat(cuota.saldo)) ).toFixed(2)); this.phuyu_vuelto();
			}
		},
		...
		phuyu_guardar: function(){
			...
			this.estado = 1; phuyu_sistema.phuyu_inicio_guardar("GUARDANDO PAGO DEL CREDITO . . .");
			this.$http.post(url+phuyu_controller+"/pagar", {"campos":this.campos,"cuotas":this.cuotascobrar}).then(function(data){
				if (data.body=="e") {
					...
					if (data.body==1) {
						phuyu_sistema.phuyu_alerta("PAGO REGISTRADO","CUOTA DE CREDITO PAGADO EN EL SISTEMA","success");
					}else{
						phuyu_sistema.phuyu_alerta("ERROR AL REGISTRAR PAGO","ERROR DE RED","error");
					}
				}
				phuyu_sistema.phuyu_fin(); phuyu_sistema.phuyu_modulo();
			}, function(){
				phuyu_sistema.phuyu_alerta("ERROR AL REGISTRAR PAGO","ERROR DE RED","error"); phuyu_sistema.phuyu_fin();
			});
		},
