var phuyu_creditos = new Vue({
	el: "#phuyu_creditos",
	data: {
		cargando: true, registro:0, codlote:0, concredito:0, buscar: "", datos: [],  sessioncaja:0, 
	...
	methods: {
		phuyu_datos: function(){
			...
			this.$http.post(url+phuyu_controller+"/lista",{"concredito": this.concredito,"buscar":this.buscar, "pagina":this.paginacion.actual}).then(function(data){
		...
		phuyu_buscar_check: function(){
			if(this.concredito==0){
				this.concredito = 1;
			}else{
				this.concredito = 0;
			}
