...
data: {
	...
	campos: {
		codkardex: 0, codpersona: 2,
		conleyendaamazonia: 1,
		codmovimientotipo: 20, codlote: 0,
...
methods: {
	...
	toggleInicial() {
		...
		if (this.inicialActivado) {
			if (this.cuotas.length > 0) {
				...
				// Recalcular las demás
				this.recalcular_fechas_desde(0);
			}
		} else {
			...
			this.$http.get(url + "ventas/pedidos/buscarproductos/" + this.codpedido).then(function (data) {
				...
				if (data.body.length == 0) {
					...
					$.each(datos, function (k, v) {
						...
						filas.push({
							codproducto: v.codproducto, producto: v.producto, codunidad: v.codunidad, unidades: this.putunidades,
	...
	phuyu_cuotas: function () {
		...
		for (var i = 1; i <= this.campos.nrocuotas; i++) {
			...
			// 	"total":total
			// });
			// Número de letra tipo 001, 002, 003...
			let nroVenta = parseInt(this.campos.nro || 0);      // 00000007 -> 7
			let nroletra = String(i).padStart(2, '0');          // 01, 02...
			let fechaCompacta = fechavence.replaceAll('-', '').slice(2); // 260316
	...
	phuyu_pagar: function () {
		...
		this.estado = 1;
		this.$http.post(url + phuyu_controller + "/guardar",
			{
