var phuyu_facturacion = new Vue({
	el: "#phuyu_facturacion",
	data: {
		sunat: {tipo:"01",serie:"",nrocomprobante:"", fdesde:"", fhasta:""}, sunatrecepcion:[],
		resumen: {codresumentipo:"", periodo:"", nrocorrelativo:0},
		facturas1:[], facturas_anuladas1:[], resumenes_boletas1:[], resumenes_info1:[], facturas_datos: [], boletas_datos: [],
		tipo_reporte: "", comprobantes_lista: [], resumenes_lista: [], guias1:[], notas_creditos1:[]
	},
	methods: {
		phuyu_comprobantes: function(){
			phuyu_sistema.phuyu_inicio();
			this.$http.get(url+"facturacion/facturacion/comprobantes").then(function(data){
				this.facturas1 = data.body.facturas; 
		...
		phuyu_notas: function(){
			this.$http.get(url+"facturacion/facturacion/notas").then(function(data){
				this.notas_creditos1 = data.body.notas;
			}, function(){
				phuyu_sistema.phuyu_alerta("NO SE PUEDEN MOSTRAR LOS COMPROBANTES PENDIENTES","","error");
				phuyu_sistema.phuyu_fin();
			});
		...
		phuyu_guias: function(){
			this.$http.get(url+"facturacion/facturacion/guias").then(function(data){
				this.guias1 = data.body.guias;
			}, function(){
				phuyu_sistema.phuyu_alerta("NO SE PUEDEN MOSTRAR LOS COMPROBANTES PENDIENTES","","error");
				phuyu_sistema.phuyu_fin();
			});
		...
		guias_enviar: function(codguiar,codoficial){
			...
			this.$http.get(url+"facturacion/facturacion/guias_enviar/"+codguiar+"/"+codoficial).then(function(data){
				if (data.body=="e") {
					phuyu_sistema.phuyu_alerta("SESION DEL USUARIO TERMINADA","DEBE INICIAR SESION NUEVAMENTE","error");
				}else{
					phuyu_sistema.phuyu_noti("ATENCION USUARIO:",data.body.mensaje,data.body.alerta);
		...
		comprobantes_xml: function(codkardex,codoficial){
			window.open(url+"facturacion/facturacion/comprobantes_xml/"+codkardex+"/"+codoficial,"_blank");
		},
		guias_xml: function(codguiar,codoficial){
			window.open(url+"facturacion/facturacion/guias_xml/"+codguiar+"/"+codoficial,"_blank");
		},
		comprobantes_cdr: function(codkardex){
			window.open(url+"facturacion/facturacion/comprobantes_cdr/"+codkardex,"_blank");
		},
		...
		phuyu_resumenes: function(){
			phuyu_sistema.phuyu_inicio();
			this.$http.get(url+"facturacion/facturacion/resumenes").then(function(data){
				this.facturas_anuladas1 = data.body.facturas_anuladas; 
				this.resumenes_boletas1 = data.body.resumenes_boletas;
				phuyu_sistema.phuyu_fin();
			}, function(){
		...
		resumenes_enviar: function(codresumentipo,periodo,nrocorrelativo){
			...
			this.$http.get(url+"facturacion/facturacion/resumenes_enviar/"+codresumentipo+"/"+periodo+"/"+nrocorrelativo).then(function(data){
				if (data.body=="e") {
					phuyu_sistema.phuyu_alerta("SESION DEL USUARIO TERMINADA","DEBE INICIAR SESION NUEVAMENTE","error");
				}else{
		...
		resumenes_xml: function(codresumentipo,periodo,nrocorrelativo){
			window.open(url+"facturacion/facturacion/resumenes_xml/"+codresumentipo+"/"+periodo+"/"+nrocorrelativo,"_blank");
		},
		...
		resumenes_ver: function(codresumentipo,periodo,nrocorrelativo){
			...
			this.$http.get(url+"facturacion/facturacion/resumenes_ver/"+codresumentipo+"/"+periodo+"/"+nrocorrelativo).then(function(data){
		...
		resumenes_quitar_ticket: function(){
			swal({
				...
				if (willDelete) {
					this.$http.get(url+"facturacion/facturacion/resumenes_quitar_ticket/"+this.resumen.codresumentipo+"/"+this.resumen.periodo+"/"+this.resumen.nrocorrelativo).then(function(data){
						if (data.body==1) {
							this.phuyu_resumenes();
