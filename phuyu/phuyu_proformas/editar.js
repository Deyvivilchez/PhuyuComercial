var phuyu_operacion = new Vue({
	...
	data: {
		...
		campos:{
			codproforma:$("#codproforma").val(),codcreditoproforma:0, codpersona:$("#codpersona").val(), codmovimientotipo:20,  
			nroreferencia:"", codcomprobantetipo:$("#comprobante").val(),seriecomprobante:$("#serie").val(), nro:$("#nrocomprobante").val(),
			fechacomprobante:"", fechakardex:"", codconcepto:13, descripcion:"REGISTRO POR PROFORMA", cliente:$("#cliente").val(), direccion:$("#direccion").val(),
	...
	methods: {
		...
		phuyu_detalle: function(){
			this.$http.post(url+phuyu_controller+"/buscarproductos/"+this.campos.codproforma).then(function(data){
				...
				preciosinigv: parseFloat(v.preciosinigv).toFixed(4), conicbper: v.conicbper, icbper: parseFloat(v.icbper).toFixed(4),
				precio: parseFloat(v.preciounitario).toFixed(4),preciorefunitario: v.preciorefunitario, porcdescuento: v.porcdescuento, 
				descuento: v.descuento,codafectacionigv: v.codafectacionigv, igv: parseFloat(v.igv).toFixed(4), 
				valorventa: v.valorventa, subtotal:parseFloat(v.subtotal).toFixed(4), subtotal_tem:v.subtotal, 
				descripcion:v.descripcion,itempedido:v.item})
		...
		phuyu_itemcalcular: function (item,tipoprecio) {
			...
			item.icbper = 0;
			if (item.conicbper==1) {
				item.icbper = Number((parseFloat(item.cantidad) * parseFloat(this.icbpersunat)).toFixed(2));
			}
			...
			this.$http.post(url+phuyu_controller+"/guardar", {"campos":this.campos,"detalle":this.detalle,"cuotas":this.cuotas,"pagos":this.pagos,"totales":this.totales}).then(function(data){
				if (data.body=="e") {
					...
					if (data.body.estado==1) {
						phuyu_sistema.phuyu_noti("PROFORMA EDITADO CORRECTAMENTE","PROFORMA EDITADO EN EL SISTEMA","success");
					}else{
        ...
        obtener_credito: function(codproforma){
        	this.$http.post(url+phuyu_controller+"/obtenercredito/"+codproforma).then(function(data){
				var datos = data.body;
				this.campos.nrodias = datos[0]["nrodias"];
				this.campos.nrocuotas = datos[0]["nrocuotas"];
				this.campos.tasainteres = datos[0]["tasainteres"];
				this.campos.codcreditoproforma = datos[0]["codcreditoproforma"];
			}, function(){
				phuyu_sistema.phuyu_alerta("ERROR AL OBTENER CREDITO","ERROR DE RED","error");
			});
        }
	},
	created: function(){
		phuyu_sistema.phuyu_fin();this.phuyu_series_referencia();this.phuyu_detalle();this.phuyu_series();
		if($("#condicionpagoref").val()==2){
			this.obtener_credito(phuyu_proformas.registro);
		}
	}
});
