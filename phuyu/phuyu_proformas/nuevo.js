var phuyu_operacion = new Vue({
	...
	data: {
		...
		campos:{
			codproforma:0, codpersona:2, codmovimientotipo:20, codcomprobantetiporeferencia:$("#comprobantereferencia").val(),seriecomprobantereferencia:$("#seriereferencia").val(), nroreferencia:"", codcomprobantetipo:$("#comprobante").val(),seriecomprobante:$("#serie").val(), nro:"",
			fechacomprobante:"", fechakardex:"", codconcepto:13, descripcion:"REGISTRO POR PROFORMA", cliente:"CLIENTES VARIOS", direccion:"-",
			codempleado:$("#codempleado").val(), codmoneda:1, tipocambio:0.00, codcentrocosto:0, nroplaca:"", retirar:true, afectacaja:true,
	...
	methods: {
		...
		phuyu_venta: function(){
			swal({
				title: "SEGURO REGISTRAR NUEVA PROFORMA?",   
				text: "LOS CAMPOS SE QUEDARAN VACIOS ", 
				icon: "warning",
				dangerMode: true,
				buttons: ["CANCELAR", "SI, NUEVA PROFORMA"],
			}).then((willDelete) => {
				if (willDelete){
		...
		phuyu_itemcalcular: function (item,tipoprecio) {
			...
			if (tipoprecio==2) {
				...
				item.preciobruto = Number((parseFloat(item.precio) + descuento).toFixed(4));
			}
		...
		phuyu_guardar: function(){
			if (this.detalle.length==0) {
				phuyu_sistema.phuyu_noti("REGISTRAR UN PRODUCTO EN EL DETALLE", "REGISTRAR ITEM PARA LA PROFORMA","error"); return false;
			}
		...
		phuyu_series_referencia: function(){
			if (this.campos.codcomprobantetiporeferencia!=undefined) {
				...
				this.$http.get(url+"caja/controlcajas/phuyu_seriescaja/"+this.campos.codcomprobantetiporeferencia).then(function(data){
					...
					this.campos.seriecomprobantereferencia = data.body.serie; this.phuyu_correlativo_referencia();
				});
		...
		phuyu_pagar: function(){
			if (this.campos.condicionpago==2) {
				if (this.campos.codpersona==2) {
					phuyu_sistema.phuyu_noti("ATENCION USUARIO: PARA REALIZAR UNA PROFORMA A CREDITO DEBE SELECCIONAR EL CLIENTE","","error");
					return false;
				}
			}
			this.estado = 1; $("#modal_cuotas").modal("hide"); phuyu_sistema.phuyu_inicio_guardar("GUARDANDO PROFORMA. . .");
			this.$http.post(url+phuyu_controller+"/guardar", {"campos":this.campos,"detalle":this.detalle,"cuotas":this.cuotas,"pagos":this.pagos,"totales":this.totales}).then(function(data){
				if (data.body=="e") {
					...
					if (data.body.estado==1) {
						this.phuyu_imprimir(data.body.codproforma);
						phuyu_sistema.phuyu_noti("PROFORMA REGISTRADO CORRECTAMENTE","PROFORMA REGISTRADO EN EL SISTEMA","success");
					}else{
		...
		phuyu_clonar: function(){
			...
			this.$http.post(url+phuyu_controller+"/clonar",{"codregistro":phuyu_proformas.registro}).then(function(data){
	...
	created: function(){
		if (parseInt(phuyu_proformas.registro)!=0) {
			this.phuyu_clonar();
		}else{
			this.phuyu_series(); phuyu_sistema.phuyu_fin(); 
		}
	}
});
