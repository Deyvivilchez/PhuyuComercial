var phuyu_editarprestamo = new Vue({
	el: "#phuyu_editarprestamo",
	data: {
		estado:0, detalle: [], entregados: [], campos:{observacion:"",codkardex:$("#codkardex").val(),codkardex_ref:phuyu_datos.registro}
	},
	methods: {
		phuyu_detalle: function(){
			this.$http.get(url+phuyu_controller+"/detalle_edicion/"+$("#codkardex").val()+"/"+phuyu_datos.registro).then(function(data){
		...
		phuyu_guardar: function(){
			...
			for (var i = 0; i < this.detalle.length; i++) {
				total = total + parseFloat(this.detalle[i]["cantidad"]);
			}
			if (total==0) {
			...
			this.$http.post(url+phuyu_controller+"/guardarcambios", {"campos":this.campos,"detalle":this.detalle}).then(function(data){
				if (data.body=="e") {
