var phuyu_salidas = new Vue({
	el: "#phuyu_salidas",
	data: {
		cargando: true, registro:0, estado:0,estadoactual:0, buscar: "",movimiento:0, datos: [], fechas:{"filtro":1,"desde":"","hasta":""}, formato_impresion: $("#formato").val(),
		paginacion: {"total":0, "actual":1, "ultima":0, "desde":0, "hasta":0}, offset: 3,
		estado_envio:0,	kardex_ref:0, codpersona:0, putunidades:[], texto_transferencia:"", detalle_prestamo:[],prestamos:0, numprestamo:"",detallecobroprestamo:[]
	},
	computed: {
	...
	methods: {
		phuyu_datos: function(){
			...
			this.$http.post(url+phuyu_controller+"/lista",{"movimiento":this.movimiento,"estadoactual":this.estadoactual,"buscar":this.buscar,"fechas":this.fechas, "pagina":this.paginacion.actual}).then(function(data){
		...
		phuyu_ver: function(){
			if (this.registro==0) {
				phuyu_sistema.phuyu_alerta("DEBE SELECCIONAR UNA SALIDA", "PARA VER EN EL SISTEMA LA SALIDA!!!","error");
			}else{
		...
		phuyu_asignarguia: function(){
			if (this.registro==0) {
				phuyu_sistema.phuyu_alerta("DEBE SELECCIONAR UNA SALIDA", "PARA REALIZAR LA GUIA DE REMISION EN EL SISTEMA!!!","error");
			}else{
				if(this.estado==0){
					return false;
				}
				$("#modal_guia").modal('show');
				this.$http.get(url+phuyu_controller+"/nuevaguia/"+this.registro).then(function(data){
		...
		phuyu_editar: function(){
			if (this.registro==0) {
				phuyu_sistema.phuyu_alerta("DEBE SELECCIONAR UNA SALIDA", "PARA EDITAR EN EL SISTEMA LA SALIDA ALMACEN !!!","error");
			}else{
				if(this.estado==0){
					return false;
				}
		...
		phuyu_imprimir: function(){
			if (this.registro==0) {
				phuyu_sistema.phuyu_alerta("DEBE SELECCIONAR UNA SALIDA", "PARA IMPRIMIR EN EL SISTEMA LA SALIDA !!!","error");
			}else{
		...
		phuyu_clonar: function(){
			if (this.registro==0) {
				phuyu_sistema.phuyu_alerta("DEBE SELECCIONAR UNA SALIDA REGISTRADA", "PARA CLONAR EN EL SISTEMA !!!","error");
			}else{
				swal({
					title: "SEGURO DESEA CLONAR ESTA SALIDA ?",   
					text: "USTED ESTA POR CLONAR UNA SALIDA", 
					icon: "warning",
					dangerMode: true,
