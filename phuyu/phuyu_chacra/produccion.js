var phuyu_operacion = new Vue({
	...
	data: {
		campos:{
			"codlote":phuyu_lineas.registro,"codmovimientotipo":10,"codpersona":1,"codcomprobantetipo":"","seriecomprobante":"","codalmacen_ref":"",
	...
	methods: {
		...
		phuyu_additem: function(producto){
		    ...
		    if (existeproducto.length==0) {
				...
				this.detalle.push({
					...
					"cantidad":1,"descuentocantidad":0,"cantidaddescontada":0,"cantidadbruta":1,"stock":producto.stock,"control":producto.controlstock,"precio":parseFloat(producto.precio).toFixed(2),
		...
		phuyu_calcular: function(producto,tipo){
			if (tipo==1) {
				...
				if (tipo==2) {
					...
					producto.cantidad = (parseFloat(producto.cantidadbruta)) - (parseFloat(producto.cantidadbruta)*parseFloat(producto.descuentocantidad)/100);
					this.totales.valorventa = Number((this.totales.valorventa - producto.subtotal).toFixed(2));
		...
		phuyu_guardar: function(){
			...
			this.estado = 1; phuyu_sistema.phuyu_inicio_guardar("GUARDANDO ENTRADA POR PRODUCCION. . .");
			
			this.$http.post(url+phuyu_controller+"/guardaringreso", {"campos":this.campos,"detalle":this.detalle,"totales":this.totales,"pagos":this.pagos,"cuotas":this.cuotas}).then(function(data){
				if (data.body=="e") {
					phuyu_sistema.phuyu_alerta("SU SESION DE USUARIO A TERMINADO","DEBE INICIAR SESION NUEVAMENTE","error");
				}else{
					if (data.body.estado==1) {
						phuyu_sistema.phuyu_alerta("ENTRADA POR PRODUCCION REGISTRADO","EN EL SISTEMA","success");
						phuyu_sistema.phuyu_modulo();
					}else{
						phuyu_sistema.phuyu_alerta("ERROR AL REGISTRAR ENTRADA POR PRODUCCION","ERROR DE RED","error");
						this.estado = 0;
					}
				}
