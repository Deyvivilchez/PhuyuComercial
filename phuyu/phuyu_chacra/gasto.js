var phuyu_nuevogasto = new Vue({
	el: "#phuyu_nuevogasto",
	data: { "rubro":$("#rubro").val(), "importetotalcredito":0,"importecredito":0,"interescredito":0,
		campos:{
			"codregistro":"","codpersona":1,"codempleado":0,"codlote":phuyu_lineas.registro,"nrotarjeta":"","fechacredito":$("#fecha").val(),"fechainicio":$("#fecha").val(),
			"nrodias":30, "codmoneda":1, "tipocambio":0.00,"nrocuotas":1,"codcreditoconcepto":2,"codcajaconcepto":7,"codtipopago":1,"fechadocbanco":$("#fechadocbanco_ref").val(),
			"nrodocbanco":"","importe":"","tasainteres":0,"interes":0,"total":0,"totales":0,"tipocuota":0,"afectacaja":true,"referencia":""
		},
		estado:0, cuotas: []
	},
	methods: {
		...
		phuyu_guardar: function(){
			...
			if(this.campos.codlote==0 && this.rubro==6){
				phuyu_sistema.phuyu_noti("ATENCION USUARIO: EL GASTO NO SE PUEDE REALIZAR PORQUE EL CLIENTE SELECCIONADO NO CUENTA CON UNA CHACRA","","error");
				return false;
			}
			this.estado = 1; phuyu_sistema.phuyu_inicio_guardar("GUARDANDO GASTO. . .");
			this.$http.post(url+phuyu_controller+"/guardargasto", {"campos":this.campos,"cuotas":this.cuotas}).then(function(data){
				if (data.body=="e") {
					phuyu_sistema.phuyu_alerta("SU SESION DE USUARIO A TERMINADO","DEBE INICIAR SESION NUEVAMENTE","error");
				}else{
					if (data.body==1) {
						phuyu_sistema.phuyu_alerta("GASTO REGISTRADO","GASTO REGISTRADO EN EL SISTEMA","success");
					}else{
						phuyu_sistema.phuyu_alerta("ERROR AL REGISTRAR GASTO","ERROR DE RED","error");
					}
				}
				phuyu_sistema.phuyu_fin(); phuyu_sistema.phuyu_modulo();
			}, function(){
				phuyu_sistema.phuyu_alerta("ERROR AL REGISTRO","ERROR DE RED","error");
			});
		},
